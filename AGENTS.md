...
- Current route certification 不得把 shadow、runtime conformance 或历史 L3 observation 改名复用为正式 route 证据。每条 route 必须有独立 request/report Schema、current generation binding、确定性 coordinator、fixture catalog、registered writer 证据和同 source revision/freeze digest 前置报告；fixture adapter 只能证明控制面，不得记为真实 Skill、provider 或人工自主执行。新认证面进入冻结集合后，旧同摘要证书立即降为 superseded 并迁入动态证据归档；仍被 tracked 状态或 replay 消费的旧报告在引用迁移前标记 historical_required，不得直接删除。
- Runtime / evaluator 认证必须先冻结产品合同、架构决定、runtime、evaluator 和 fixture 五类显式文件摘要，并在隔离 `state/checks/` work root 内运行。认证前后总摘要必须一致；同一固定工作目录不得被并行认证共享。`compile_smoke`、未提交 worktree、旧专项 checker 绿灯或缺少完整 commit SHA 时只能写 `not_run / not_certified`，不得晋升为认证证据。
- 点加载带脚本级 `param(...)` 的 PowerShell helper 可能覆盖调用入口同名变量；新增入口必须在点加载前保存用户参数到专用名称，或把 helper 改为无脚本参数的纯函数模块，并用显式 `-OutputPath` fixture 验证参数未被污染。
- PowerShell checker 在 `Set-StrictMode` 下读取可选 JSON / YAML 字段前必须先检查属性存在；缺少可选字段应走产品定义的默认/不适用分支，不能升级为 checker `PropertyNotFoundStrict` 工具错误。
