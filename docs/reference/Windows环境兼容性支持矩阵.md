...
| Windows ARM64 | `workflow_compiled_remote_run_pending` | GitHub job 已使用 `windows-11-arm`，需 push 后核对同 commit run |
| Windows Server | `workflow_compiled_remote_run_pending` | GitHub jobs 已显式使用 `windows-2022`、`windows-2025`，需 push 后核对同 commit run |
...
github_hosted_jobs:
  windows_server_2022: compiled_not_run
  windows_server_2025: compiled_not_run
  windows_11_arm64: compiled_not_run
system_configuration_mutated: false
```
