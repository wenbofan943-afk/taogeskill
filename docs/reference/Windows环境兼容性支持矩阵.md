...
> 状态：`windows_powershell_5_1_current_baseline`
> 证据日期：2026-07-13
...
| 项目 | 当前口径 |
|---|---|
| 当前公开兼容宿主 | Windows PowerShell 5.1；本轮实测 5.1.26100.8737 |
| PowerShell 7 | 不属于当前公开支持、安装前置或发布阻断；旧实测记录仅保留历史审计语义 |
| 安装根 | 建议完整路径不超过 90 字符 |
...
H5 当前 full matrix：Windows PowerShell 5.1 的 6/6 符合预期，fail=0，not_tested=0。4 个正例都运行 runtime-helper 和 environment-preflight；ZIP 正例还验证包内相对路径、必需文件、count、size 与 SHA256。2 个超预算 case 没有创建目标目录。

PowerShell 7.6.3 的旧结果可在本文件历史章节和 Git 发布记录中审计，但不构成当前版本支持宣称。

## 3. 历史 Alpha 证据（不构成当前支持承诺）

以下记录描述既有 alpha 发布时的真实验证，保留供事故追溯；它们不改变本文件前两节定义的 Windows PowerShell 5.1 当前基线。
...
## 3.6 H7 loopback SMB 与远端 runner 编译
