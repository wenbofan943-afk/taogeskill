...
| Windows ARM64 | `remote_validation_debugging` | `windows-11-arm` 已产生真实 runner 证据；前两轮 full matrix 暴露宿主默认编码泄漏，修复后仍需同 commit 重跑成功才能认证 |
| Windows Server | `remote_validation_debugging` | `windows-2022` / `windows-2025` 已产生真实 runner 证据；前两轮 full matrix 暴露宿主默认编码泄漏，修复后仍需同 commit 重跑成功才能认证 |
...
github_hosted_jobs:
  windows_server_2022: diagnostic_failed_fix_pending_rerun
  windows_server_2025: diagnostic_failed_fix_pending_rerun
  windows_11_arm64: diagnostic_failed_fix_pending_rerun
diagnostic_runs:
  - 29200062878
  - 29200531310
