...
13. `invoke-windows-clean-room-matrix.ps1 -Mode full` 的 6 个 Windows PowerShell 5.1 × path × source/zip case 全部符合预期；definition-only 只证明矩阵完整，不能替代执行证据。PowerShell 7 不属于当前公开承诺；若重新列入支持范围，必须先升级产品合同与矩阵。未覆盖轴进入 known limits，不得被 overall pass 吞并。
