...
| `environment_compatibility_gate` | PowerShell、外部进程、路径、压缩包、构建器、安装或公开包发生变化 | 5.1/7 × short/空格中文/超预算 × source/zip 12-case 矩阵逐项符合预期；正例运行代表 checker，超预算在副作用前阻断；缺宿主 / 未覆盖轴标记 not_tested / not_certified | 不得以当前机器短路径、单一 pwsh CI 或 definition-only pass 宣称 Windows 兼容；回到环境合同、case fixture 或 CI | `tools/invoke-windows-clean-room-matrix.ps1 -Mode full` + `P3REL-029` |
