...
| `environment_compatibility_gate` | PowerShell、外部进程、路径、压缩包、构建器、安装或公开包发生变化 | Windows PowerShell 5.1 × short/空格中文/超预算 × source/zip 六格矩阵逐项符合预期；`validate-gates` 必须实际解析最新 full 报告，缺报告为 blocked；扩展轴先由环境探针确认真实 host/root，再在同一 commit/hash 上运行 full matrix + public validator；PowerShell 7 不属于当前支持目标 | 不得以当前机器短路径、路由中出现 gate 名、环境标签、synthetic fixture、definition-only 或旧绿灯宣称 Windows 兼容；回到 gate handler、环境合同、case fixture、hosted/self-hosted CI | `tools/validate-gates.ps1 -GateName environment_compatibility_gate` + `tools/invoke-windows-clean-room-matrix.ps1 -Mode full` + `P3REL-029/P3REL-031` |
