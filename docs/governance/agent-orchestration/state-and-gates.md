...
| `product_contract_compilation_gate` | 产品规则含数量、默认值、条件、成本、调用数或派生状态；或已编译合同被新确认产品定义取代 | 按 `compilation-control.md` 的六层闭合：产品与字段、执行合同、机器合同、runtime、正反 fixture、专项 checker 同源；fixture 常量与通用规则分离；被取代的旧 sink 全部标记 `superseded_pending_recompile`，旧 checker pass 只算历史兼容 | 回到合同编译，不允许用 prose、magic number、手工运行目录补件或旧 checker pass 冒充新实现；重编译前阻断依赖旧合同的真实外部回归 | C71-C80：`validate-r3-visual-need.ps1`；C81-C90：`validate-p0-h6-reliability.ps1`；R7-C133-C160 H1：`validate-r7-l3-h1-evidence.ps1`；旧 visual-budget 只验证历史兼容 |
