...
| `environment_compatibility_gate` | PowerShell、外部进程、路径、压缩包、构建器、安装或公开包发生变化 | 基础 5.1/7 × short/空格中文/超预算 × source/zip 12-case 矩阵逐项符合预期；扩展轴先由环境探针确认真实 host/root，再在同一 commit/hash 上运行 full matrix + public validator；缺基础设施标记 blocked_external_infrastructure | 不得以当前机器短路径、单一 pwsh CI、环境标签、synthetic fixture 或 definition-only pass 宣称 Windows 兼容；回到环境合同、case fixture、hosted/self-hosted CI | `tools/invoke-windows-clean-room-matrix.ps1 -Mode full` + `tools/invoke-windows-certification-probe.ps1` + `P3REL-029/P3REL-031` |
