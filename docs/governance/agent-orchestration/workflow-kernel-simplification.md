...
> 当前状态：`m6_evaluator_conformance_suite_compiled_not_certified`
> 目标：停止继续堆叠 current 蓝图、注册表和专项补丁，把现有能力收敛为一个轻量、可恢复、可替换的本地工作流内核。
> 确认：用户于 2026-07-18 认可 `ARCH2-D01` 至 `ARCH2-D10`。
> 边界：M1-M3 已完成 shadow 编译，M4 完成未来新 session 的代际切换，M5 完成 current/legacy 兼容隔离，M5.1 完成仍有消费者资产的目录级归档。M6 已编译五类冻结和 evaluator conformance suite，但只运行 compile smoke，evaluator/runtime certification 均未运行；既有 session 未迁移，历史合同未删除，未联网、调用外部 provider 或修改 R8 产品草案。
...
已通过 Windows PowerShell 5.1 compile smoke；前后总摘要一致。该运行绑定
`git_worktree_pending_commit`，所以正确状态仍是
`not_run_compile_smoke_only`，不构成 evaluator certification。
