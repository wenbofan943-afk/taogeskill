...
> 边界：M1-M3 已完成 shadow 编译，M4 完成未来新 session 的代际切换，M5 完成 current/legacy 兼容隔离，M5.1 完成仍有消费者资产的目录级归档。M6 已编译五类冻结、evaluator conformance 与 runtime conformance suite；runtime suite 接入后必须在新 clean HEAD 上重跑同摘要 evaluator/runtime certification。既有 session 未迁移，历史合同未删除，未联网、调用外部 provider 或修改 R8 产品草案。
...
通过 compile smoke；随后在 clean HEAD `30be209…` 上完成一次 evaluator
certification。M6 runtime suite 接入后冻结集合发生变化，该旧证书保留审计
但不授权新 runtime suite。
