...
| 产品开发 / 产品确认 | [产品文档索引](./product/README.md) | 对应 current 产品组、确认清单或路线图 |
...
| 追溯旧合同或历史决定 | [归档索引](./archive/README.md) | 只在 replay、迁移或审计时按需读取 |
...
当前解释：docs/explanation
历史审计：docs/archive、路线图历史章节
```
...
| `archive/` | 已被取代但仍需审计或 replay 的历史文档 | current route 必读、当前合同真源 |
| `how-to/` | 面向人类的操作指南 | 产品决策过程 |
| `tutorials/` | 脱敏可公开教程和完整样例 | 真实账号 / 真实 run |

## 索引维护规则

新增、移动或重命名知识文档时，先更新所属目录的 `README.md`；只有新增入口级能力才更新根 `README.md` / `PROJECT_MAP.md`。目录索引负责“完整覆盖”，根入口负责“最短路径”，避免两个根文件重复维护全量清单。归档文档必须进入 `archive/` 分区索引，不能继续占用 current 产品或解释索引。
