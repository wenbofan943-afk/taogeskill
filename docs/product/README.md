...
当前确认范围：R0 至 R4 已完成既定编译与公开交付治理；`v0.1.0-alpha.6` 已作为 GitHub 预发布版本发布。R3-C01 至 C124 已完成产品确认；C91-C124 已把视觉插入呈现方式、结构化画幅 / 槽位、平台 rendition、独立封面适配、表面预览和显式视觉门禁编译为 v0.4 合同。旧 P0-H7 v0.3 仅保留 replay / 原版复现，新交付必须创建 v0.4 revision。最新真实直供稿的旧 revision 因横向正文图被盲目居中裁成竖封面，人工验收仍为 fail / 不可发布；不能用本轮脱敏 fixture 的绿灯覆盖该真实结论。R5-H1 至 H6 已完成账号视觉、二手车优先雷达、四层热点对象、扩词反馈、账号启动检查 / 最小补问 / session 账号快照，以及跨账号技术身份绑定与自动校验的合同编译；私有账号显式身份绑定迁移与真实启动回归已通过。R6-C01 至 C19 已完成“用户直供文案”与“新闻证据画中画”的本地六层编译，R6 专项 17/17 通过；跨入口写作质检与内容设计编排增强登记为 `R6-B01` 后续产品开发，不纳入本轮完成声明。R6 尚未进入新 GitHub Release，真实热点检索仍须由用户发起具体内容任务。
