...
当前确认范围：R0 至 R4 已完成既定编译与公开交付治理；`v0.1.0-alpha.6` 已作为 GitHub 预发布版本发布。R5-H1 至 H6、R6-C01 至 C50 与 R3-C01 至 C139 当前产品合同已完成本地编译。一轮私有真实稿件回归证明 renderer 可重复运行，也暴露语义编排和 candidate 高扶跑；R7-C01 至 C28 已确认，R7-H1 至 H3 已完成合同、状态提交与直供 producer adapter 本地编译，H4-H6 继续按真实回归闭环推进。该批尚未进入新 GitHub Release。
