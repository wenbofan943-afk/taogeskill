...
当前确认范围：R0 至 R4 已完成既定编译与公开交付治理；`v0.1.0-alpha.8` 是当前已发布 GitHub 预发行版本。R5-H1 至 H6、R6-C01 至 C60、R3-C01 至 C163 与 R7-C01 至 C132 已确认并完成本地分批编译。H7A 已闭合唯一最终素材、业务优先 HTML、技术 viewport 与独立业务验收合同；H7B 已用新的私有 session 完成业务验收与退出码修复复测，项目仍不构成 L3。R8-C01 至 C70 已确认，H1-H4 已完成 28 个业务 Skill 的上下文 inventory、热点入口、传播 router、内部 human gate 和平台包装渐进披露编译。H5 v0.1 经递归审计判定为无效业务 A/B，旧匿名包不进入人类评审；H5R1 已完成九类对象 Schema、字段词典、兼容状态和正反 fixture，H5 v0.2 当前为 `schema_compiled_adapters_pending`。
