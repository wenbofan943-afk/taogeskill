...
当前确认范围：R0 至 R4 已完成既定编译与公开交付治理；`v0.1.0-alpha.6` 已作为 GitHub 预发布版本发布。R5-H1 至 H6 已完成账号视觉、二手车优先雷达、热点对象、扩词反馈、账号启动与跨账号身份绑定编译。R6-C01 至 C50、R3-C01 至 C139 已完成本地编译；当前新运行使用直供 baseline、短视频结构、全文节点、口播质检、视觉覆盖账本、对齐审查和 P0 v0.5 最终交付。一轮私有真实稿件回归证明 renderer 可重复运行，也暴露语义编排和 candidate 高扶跑；R7-C01 至 C28 已确认，R7-H1 合同底座已本地编译，H2 coordinator runtime 以后仍待独立编译。该批尚未进入新 GitHub Release。
