...
```text
docs/archive/product/内容创作质量方法论编译补充-R1.md
docs/archive/product/R1-P14-方法论编译规则.md
docs/archive/product/R1-P15-skill粒度与入口治理规则.md
docs/archive/product/R1-P13-execution-trace检查清单与validator草案.md
docs/archive/product/R1-P02-agent扶跑收敛与可编译判定.md
docs/archive/product/R1-skill执行合同组可编译总验收.md
docs/archive/product/R1-合同版本与变更治理.md
docs/archive/product/R1-字段级输入输出矩阵.md
docs/archive/product/R1-人类门禁决策枚举与恢复规则.md
docs/archive/product/R1-trace-check注册表.md
docs/archive/product/R1-产品确认清单.md
```
...
docs/archive/product/R4-产品确认清单.md
```
...
> 定义时间：2026-07-07
> 历史产物：`docs/archive/product/R1-R4只读checker产品定义.md`
...
| 编译项 | 结果 |
|---|---|
| 产品定义 | `docs/archive/product/R1-R4只读checker产品定义.md` 状态改为 confirmed_and_compiled，现仅供历史审计 |
...
> 记录时间：2026-07-07
> 历史产物：`docs/product/R4-开源交付与净化规则.md#11-r4-public_release-candidate-前置确认记录`、`docs/archive/product/R4-产品确认清单.md#7-r4-public_release-candidate-前置确认`
...
> 记录时间：2026-07-11
> 触发：已脱敏标识的真实回归证明现有 replay 只能回看证据，不能作为实际执行边界；此前把该缺口直接写成脚本属于跳过产品定义，已回滚。
...
| `P0-F07` | 旧私有回归类 session（标识已脱敏） | `legacy_evidence_replay`，保留 agent_assist_level=high |
...
两个阶段都必须新建 regression session，不能修改已完成的私有历史 run。当前默认基线是最新已验证真实回归（标识已脱敏）；执行前若已有更新且同等完整的真实 run，可在 preflight 中改选，但必须记录原因。
...
**历史 H6 prompt preflight**：H5 的 `intermediate/05-visual-plan.md` 的确只保留 prompt ID 和验收摘要，但更早的私有真实来源 session（标识与路径已脱敏）保存了 1 张封面底图和 3 张画中画的实际完整 prompt。完整 prompt / digest 证据继续有效；“3 张画中画自动 accepted、最多 4 次调用、cost gate”已被 8.15.20 取代。
...
**执行结果**：在全新私有 session `PRIVATE-REGRESSION-REDACTED` 复用 H5 已验证口播，不复用旧 3+1 数量结论。H6A 按 8 个语义节点生成 visual need analysis，结果为 8 accepted / 0 rejected；所有 accepted task 均证明了 Hook、理解、演示、情绪或记忆价值，`derived_visual_count=8`。analysis pass 后未设置人工确认停点，自动进入 H6B。
...
**触发问题**：`PRIVATE-REGRESSION-REDACTED` 的 HTML 文件、图片、链接、hash、receipt、event、projection 和 resume 均闭合，但用户查看页面后触发一轮只读业务审计。审计证明“技术渲染成功”没有等于“业务交付成功”：
...
| P0-H7-C15 | 新增跨产物业务语义门禁，并重建 `PRIVATE-REGRESSION-REDACTED` 验收；只有平台绑定、revision、warning、插入点、时长与用户层全部通过，才能恢复 human-ready 结论。 |
...
-> rebuild PRIVATE-REGRESSION-REDACTED current revision
...
**P0-H7-H1 编译结果**：已新增 plan / typed input schema v0.3、compatibility matrix、final-delivery renderer/template、H7 prepare / finalize、公开 fixture 与语义 checker，并接入 field / product / runtime / public release gates。真实 `PRIVATE-REGRESSION-REDACTED` 复用 H6 已验证的 8 张 PIP，不新增 Image 2 调用；按 4 个平台发布单元重合成 3 张封面，从 `DREV-PRIVATE-REDACTED` 同源生成五个最终视图，并将 projection / resume / manifest 收口到 H7 event tail。脱敏 fixture 10/10，真实语义 checker 20/20，桌面 / 移动可视审计通过，结果 `pass_with_warnings`；未发布和传播效果仍为 not tested。
...
**R4-C41 到 C58 产品合同**已经进入 [R4 产品确认清单归档](../archive/product/R4-产品确认清单.md)。本段记录的旧核心支持口径是：PowerShell 7 推荐；Windows PowerShell 5.1 保留根目录不超过 90 字符的短路径兼容。该口径现已被 R4-C59 / C60 取代：当前只承诺 Windows PowerShell 5.1 六格矩阵。其余路径、预检和 `not_certified` 规则仍有效。
...
>
> 批次状态：`R8-C01-C70_confirmed_H5_v01_invalid_H5R5_finalized_with_blockers`
