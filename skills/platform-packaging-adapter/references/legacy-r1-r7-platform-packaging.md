...
source_contract: pre-R8-H4 embedded platform packaging entry
---
...
primary_output: platform_package_input + platform_package + cover_variant_set + content_delivery_record
next_skill_on_pass: cover-design-compiler
```
...
必须先编译 platform_package_input，再生成 platform_package 和 cover_variant_set。
package_pass 后必须生成 content_delivery_record，并自动进入 cover-design-compiler；不得停在“确认采用”。
```
...
delivery_status=delivery_ready 时 next_skill=cover-design-compiler。
...
Step 2：基于 platform_package_input 生成多平台分发包和 cover_variant_set。
Step 3：生成 content_delivery_record，并把封面策略交给 cover-design-compiler。
```
...
封面标题也不等于封面成品。每个平台必须输出供下游编译的封面策略信息：

```text
cover_image_source：existing_image_asset / generated_cover / video_frame / manual_required / prompt_only
cover_image_source：底图来源建议，不代表成品已生成
cover_visual_concept_hint：视觉入口和点击任务
platform_cover_strategy_hint：reuse / crop / retitle / independent_composition / prompt_only
cover_layout_hint：主体、文字、留白、视觉焦点
platform_cover_notes：平台差异和安全区提示
```
...
7. 包装通过后生成 `content_delivery_record`，自动进入 `cover-design-compiler`；最终 HTML 仍负责把“可确认、可返工、可归档、可放弃”的选项说成人话。
...
cover_variant_set 或封面策略输入缺失 -> 不进入 cover-design-compiler，先补平台封面策略
...
平台包装通过 -> 生成 content_delivery_record，并自动进入 cover-design-compiler；不再等待“确认采用”
```
...
```text
平台包装已经完成，我会继续生成封面成品、做封面专项质检，再生成最终 HTML 验收页，不需要你再回复“确认采用”。
...
auto_next_action：平台包装通过后自动进入 `cover-design-compiler`；收到局部修改指令后只回退对应环节。
