...
primary_input: draft + static_visual_director_plan + visual_need_analysis + visual_plan + visual_text_plan + image_asset_set + optional R6 evidence bundle
primary_output: quality_review / visual_text_quality_gate / cover_quality_gate
...
```text
content_visual_review：draft + visual plan + visual text plan 初审；同时生成 visual_text_quality_gate；cover_quality_gate_status=not_applicable；通过后进入 platform-packaging-adapter。
...
visual_need_analysis 必须验证每个 generate 候选的 `viewer_problem_without_visual / primary_visual_job / expected_viewer_change`，并确认 accepted task 没有被数量、成本或 provider call limit 截断。
`derived_visual_count=0` 时必须检查 `zero_visual_reason` 是否基于人物表现、口播具体性和受众理解，而不是时长、成本或调用次数。
visual_text_quality_gate_status 必须逐图检查 decision、information_delta、文字预算、可读性、准确性和来源绑定。
...
generated required 图片缺 generation_record，不得 review_pass；source_capture 证据图改查 source_capture_record + evidence_claim_binding + evidence sidecar。
...
forbidden 任务出现文字、required 任务缺字、文字只复写口播 / 字幕，不得通过。
source-required unit 缺 source type / id / path / source_bound，不得通过。
生成情境图被标为 evidence_support，不得通过。
source_capture 证据图缺 publisher / canonical_url / selected_target / capture hash / supported binding / 来源与解读分层，不得通过。
```
...
```text
读：content_brief、draft、visual_need_analysis、visual_plan、visual_text_plan、image_asset_set；存在 source_capture 时另读 claim_card、source_record、source_capture_record、evidence_claim_binding 和 evidence sidecar。
取：从 draft 取 script_text、hook_route、body_information_density_score、core_mechanism；从 visual_plan 取首屏任务和图片状态。
传：quality_review 必须带 review_mode、review_id、draft_id、visual_plan_id、content_source_id、content_origin、review_status、blocking_issues、risk_review、must_fix_segments、artifact_path、next_skill；热点入口另带 source_research_run_id。
传：content_visual_review 必须带 visual_quality_gate_status、static_visual_quality_gate_status、visual_text_quality_gate_status、prompt_integrity_status、image_asset_trace_status、asset_trace_quality_gate_status、html_embed_readiness_status，并明确 cover_quality_gate_status=not_applicable。
传：每个 visual_text_quality_gate 必须带 image_task_id、image_asset_id、information_delta_status、narration_redundancy_status、subtitle_redundancy_status、glance_comprehension_score、mobile_readability_score、cognitive_load_status、source_binding_status、text_accuracy_status、quality_gate_status、recovery_action、next_skill。
...
是否有必须画中画却漏掉的段落。
是否每个 generate candidate 都进入 accepted_visual_tasks，且所有 accepted tasks 都有 prompt / generation record；不得只生成前四张。
是否存在仅因 elapsed time、纯装饰、重复或情绪错位而错误 accepted 的任务。
...
static_visual_quality_gate_status = pass：静态视觉编导方案能说明视觉角色、构图策略、风格锚点和每张图的视觉需求证明；0 图时 zero_visual_reason 成立。
...
prompt_integrity_status = fail -> image-prompt-compiler
image_asset_trace_status = fail -> image-asset-producer
static_visual_quality_gate_status = fail -> static-visual-director
visual_text_quality_gate_status = fail -> recovery_action 指定 static-visual-director / image-prompt-compiler / image-asset-producer
cover_review 且 cover_quality_gate_status = fail -> cover-design-compiler
asset_trace_quality_gate_status = fail -> image-asset-producer / final-delivery-builder
html_embed_readiness_status = fail -> image-asset-producer / final-delivery-builder
