...
账号存在时，任何热点、选题、内容或视觉任务先调用 `tools/invoke-account-startup-check.ps1`：只问当前任务真正缺失的字段，一轮最多 3 个口语问题；用户回答后冻结 session 账号快照。不得从历史文章、前一账号或聊天印象猜平台、时长、受众优先级或高风险口径。
...
传：传给下游时必须带 session_id、account、product_profile_id 或 campaign_profile_id、source_research_run_id、artifact_id、artifact_path、next_skill。
...
entry_intent：
entry_phrase：
entry_case：first_use_no_account / existing_account_run / switch_account / resume_last / checker_only / image_capability_question / unknown_entry
entry_route：account-onboarding / propagation-router / hotspot-topic-research / workflow-checker / capability_notice / human_confirm
...
account_startup_check=account_ready -> 写本 session account_snapshot_ref；再检查 product_profile / campaign_profile
