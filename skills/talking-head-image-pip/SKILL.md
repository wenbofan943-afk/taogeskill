...
primary_output: static_visual_director_plan + visual_plan + image_prompt_set + image_generation_record + image_asset_set
next_skill_on_pass: copywriting-quality-review
...
```text
本 skill 在 R3 中不只产“画中画提示词”，还必须先产出静态视觉编导方案，再产出可追溯图片资产链。
...
不得直接从内容语言跳到图片 prompt；必须先有 `static_visual_director_plan`。
每张图片必须区分 `picture_in_picture_image` / `cover_image`，并区分 `codex_image2_render` / `seedream_prompt_delivery`。
本 skill 只生产 `cover_background_asset`，不把底图称为成品封面；叠字、裁切和平台成品由 `cover-design-compiler` 负责。
```
...
image_prompt_set_id：
image_asset_set_id：
image_asset_type_summary：picture_in_picture_image / cover_image
cover_asset_role_summary：not_applicable / cover_background_asset
image_production_path_summary：codex_image2_render / seedream_prompt_delivery / manual_upload / not_available
image_generation_decision：render_now / deliver_prompt_only / manual_required
prompt_delivery_mode：html_copyable_prompt / prompt_card_md / external_model_payload / not_required
required_count：
optional_count：
generated_count：
pending_count：
...
```text
本 skill 负责静态视觉编导、画中画视觉计划、提示词、图片生成记录和图片状态；不写正文、不做质检、不做平台包装。
...
传：static_visual_director_plan 必须带 static_visual_director_plan_id、draft_id、brief_id、source_research_run_id、visual_role_map、style_anchor、composition_strategy、image_asset_type_plan、static_visual_director_status、artifact_path、next_skill。
传：visual_plan 必须带 visual_plan_id、static_visual_director_plan_id、draft_id、brief_id、source_research_run_id、pip_insert_map、image_prompt_set、image_generation_record、image_asset_set、image_assets_status、visual_plan_status、artifact_path、next_skill。
传：每张 image_asset / generation_record 必须带 image_asset_type、cover_asset_role、image_production_path、image_generation_decision、prompt_delivery_mode；非 Codex 路径必须带 external_model_payload_path 或明确 not_available 原因。
传：picture_in_picture_image 写 cover_asset_role=not_applicable；封面底图写 cover_asset_role=cover_background_asset。不得在本 skill 写 cover_composited_asset / platform_cover_asset。
...
### 1.5 Build Static Visual Director Plan

Before writing prompts, compile:
...
static_visual_director_status：director_plan_pass / director_plan_needs_fix / director_plan_blocked
```

This layer turns copy into visual language. It must name the visual role of each planned image:

```text
conflict / evidence / emotion / method / trust / transition / memory / cover_click
```

If the plan only repeats content text, slogans, or abstract concepts without composition, subject, scene, and retention task, set `static_visual_director_status=director_plan_needs_fix`.
...
### 4.5 Decide Image Asset Type

For every planned image, set:
...
```text
picture_in_picture_image：插入口播正文，必须有 beat_id、retention_task、insert_after_text、insert_before_text。
cover_image：进入封面设计包，必须服务点击和入口理解，可引用已有画中画图，也可独立生成。
```

Do not use one generic "image" bucket. If one file is reused as both, keep both roles in metadata and make both quality gates explicit.
...
### 6.5 Provider-Adaptive Output

The 14-layer planning card is provider-agnostic. At output time, compile to the target provider format.

Before output, decide:

```text
image_production_path: codex_image2_render / seedream_prompt_delivery / manual_upload / not_available
image_generation_decision: render_now / deliver_prompt_only / manual_required
prompt_delivery_mode: html_copyable_prompt / prompt_card_md / external_model_payload / not_required
```
...
```text
image_production_path=codex_image2_render
provider_mode=codex_builtin
provider=gpt-image-2
image_generation_decision=render_now
```

Non-Codex environment:

```text
image_production_path=seedream_prompt_delivery
provider_mode=not_available unless a reviewed external adapter exists
...
Non-Codex output must include a Seedream payload. It must not claim that an image was generated.

**Codex / GPT Image 2 (default):**
Use the five-slot labeled format from section 6. Include `Model: gpt-image-2`, `Quality`, `Size`.

**Seedream (4.0 / 4.5 / 5.0):**
Compile the 14-layer card into a natural-language paragraph. Do not use labeled slots or five-slot structure. Seedream is a native Chinese model that prefers coherent paragraphs over structured fields.

Compilation rules (14-layer → Seedream):

| 14-layer field | Seedream compilation |
|---|---|
| 主体 / 动作 | 直接写入 prompt 开头：`"一位中年老板坐在办公桌前看手机"` |
| 场景 / 环境 | 紧跟主体后：`"深夜办公室，落地窗外城市夜景"` |
| 镜头 / 构图 | 转为自然语言描述：`"近景，低角度拍摄，右三分之一留白"` |
| 光线 / 情绪 | 融入描述：`"手机屏幕光从右下45°打上来形成底光，冷白调"` |
| 五槽提示词 | 全部打平为一个连贯段落，不使用 Scene / Subject 等标签 |
| 中文提示词 | 作为主 prompt 输出（Seedream 中文理解强，直接使用中文描述） |
| 负面提示 | 转为肯定指令：`"不出现文字"` → `"画面干净无文字"` |
| 模型 / 质量 / 画幅 | 转为独立 API 参数，不在 prompt 正文中重复 |

Seedream API 参数映射：

```yaml
prompt:          # 自然语言段落（见下方示例）
aspect_ratio:    "9:16" / "16:9" / "1:1"  # 取自原画幅规划
size:            "2048x2048"               # 默认 2K
watermark:       false                     # 去水印
enhance_prompt:  true                      # 开启自动优化
```
...
| 场景 | 推荐版本 |
|---|---|
| 成本敏感、快速出图 | seedream-4.0 |
| 需要画面文字、品牌级质量 | seedream-4.5 |
| 复杂语义、逻辑图表、需联网参考 | seedream-5.0 |

Seedream prompt 完整示例（将 14 层编译为自然语言）：

```text
一位中年老板深夜独自坐在办公桌前，低头看着手机，手机屏幕的光映在疲惫的脸上形成底光效果，颧骨和鼻梁高光明显，落地窗外是模糊的城市夜景。近景，低角度，右三分之一留白，冷白色灯光，写实纪录片风格，画面干净无文字。9:16竖屏。
```

对应的 API 入参：

```yaml
prompt: "一位中年老板深夜独自坐在办公桌前，低头看着手机，手机屏幕的光映在疲惫的脸上形成底光效果，颧骨和鼻梁高光明显，落地窗外是模糊的城市夜景。近景，低角度，右三分之一留白，冷白色灯光，写实纪录片风格，画面干净无文字。9:16竖屏。"
aspect_ratio: "9:16"
size: "2048x2048"
watermark: false
enhance_prompt: true
...
完整性检查：Seedream prompt 同样适用 `prompt_integrity_check`，但检查标准不同：

```text
GPT Image 2 → 检查五槽是否齐全、labeled section 是否完整
Seedream    → 检查主体、场景、风格、技术参数四要素是否都在段落中
```
...
beat_id：
image_asset_type：
cover_asset_role：not_applicable / cover_background_asset
...
image_prompts_count：
image_asset_set_id：
image_asset_type_summary：
cover_asset_role_summary：
image_production_path_summary：
image_assets_status：
generation_records_dir：
metadata_dir：
...
During final delivery, render required images before producing `deliverables/final-delivery.html`. If rendering is unavailable (non-Codex environment), do not block the whole delivery page; write `image_status = pending_external` or `image_status = generation_failed`, include the **Seedream-format prompt** compiled via section 6.5, and show the missing-image status with copyable prompt in the HTML.
...
For non-Codex environments, this skill actively outputs Seedream-format prompts (section 6.5). The `provider` field in `image_generation_record` must reflect the target provider: `seedream-4.0`, `seedream-4.5`, or `seedream-5.0`; `image_production_path` must be `seedream_prompt_delivery`. This skill must not call external image APIs unless the project later adds a dedicated, reviewed adapter.
