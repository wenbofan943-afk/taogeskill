...
primary_input: content_delivery_record + visual_text_plan + visual_text_quality_gate + image_asset_set + cover_design_package + cover_composition + cover_quality_gate + optional R6 evidence records
...
## P0-H4 Evidence Runtime

```yaml
contract_version: 0.9.0
contract_status: h4_evidence_runtime_active
...
H2 证明脱敏单篇 compiler / renderer；H3 已用 P0-F03 至 F19 独立 fixture 固化等待、失败、恢复、兼容、并发、中断和取消验收。H3 不代表 H4 的 event writer / reconciliation 命令已实现；真实已验证图片回归仍属于 H5。
...
## P0-H7 Visual Presentation Delivery Revision
...
legacy_policy: v0.2_v0.3_readonly_replay_and_reproduction
```

H7 把最终 HTML 定位为发布执行工作台。上游先生成 `typed_components_v0.4` candidate；compiler 重新派生 `platform_delivery_scope_status`，renderer 从同一 revision 生成 HTML、最终文案、最终视觉方案、最终平台包和内容交付记录，最后才写 `delivery-revision.json` commit marker。每个平台必须绑定独立 cover rendition、`preview_evidence_type`、真实封面成品、平台表面预览和显式视觉审核记录；`cover_delivery_status=visual_pass` 只有 `codex_visual_review` 或 `human_visual_review` 能写。视觉插入统一使用 `visual_insert`，并显式展示 `presentation_mode`、视频画布、资产画布和 `placement_slot`；`speaker_plus_visual` 才是狭义画中画。主平台通过、次平台待审时只能写 `primary_ready_secondary_pending`，不得宣称全部平台可发布。模板或 renderer 变化必须进入渲染 operation digest，不能错误复用旧页面。
...
新闻证据画中画必须在资产本体或卡片中清楚分开“来源事实”和账号解读，并在追溯区显示 publisher、页面标题 / 日期、canonical_url、capture_at、capture hash、claim_evidence_status 与 binding_id；不得把 source_capture 计为 Image 2 生成。
```
...
```text
读：manifest、content_delivery_record、draft、visual_plan、visual_text_plan、quality_review、visual_text_quality_gate、platform_package、cover_design_package、cover_composition、cover_quality_gate、image_asset_set；存在 source_capture 时另读 claim/source/capture/binding/evidence sidecar。
...
传：final_delivery 必须带 final_delivery_id、delivery_id、content_source_id、content_origin、entrypoint_path、delivery_page_mode、final_delivery_status、image_assets_status、visual_text_plan_id、visual_text_quality_gate_status、visual_text_delivery_summary、cover_composition_ids、cover_embeds、upload_ready_cover_count、prompt_only_cover_count、html_embed_manifest_status、export_status、artifact_path、next_skill；热点入口另带 source_research_run_id。
...
6. 追溯材料：当前 content source（topic_card 或 direct_content_card）、content_brief、draft、visual_plan、visual_text_plan、visual_text_quality_gate、quality_review、platform_package、cover_design_package、delivery_record；有证据截图时另含 source/capture/binding/evidence sidecar。
```
