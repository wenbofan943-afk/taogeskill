...
## R1 Compatibility Runtime

```yaml
contract_set_version: r1-contract-set-v0.1
contract_status: compatibility_entry
skill_type: compatibility
primary_output: router_handoff
```

本 skill 是旧唤醒词兼容入口，不承载 R1 新逻辑，不生产正式 artifact。
按 `docs/reference/R1-skill渐进读取与长文边界.md` 执行渐进读取；本入口只读兼容 Runtime 和路由表，不读取专项 skill 全文来代跑。

处理规则：

```text
用户要总控 / 不知道下一步 -> 转 propagation-router。
用户要找热点 / 评选题 -> 转 hotspot-topic-research。
用户已选题 -> 转 content-brief-compiler。
用户已有 Brief 要写稿 -> 转 copywriting-draft-writer。
用户要画中画 -> 转 talking-head-image-pip。
用户要质检 -> 转 copywriting-quality-review。
用户要平台标题 / 描述 / 话题 -> 转 platform-packaging-adapter。
用户要最终 HTML / 转交包 -> 转 final-delivery-builder。
```

禁止：

```text
禁止在本 skill 内补字段、写正文、出图、质检、平台包装或最终交付。
禁止绕过 account_profile、product_profile / campaign_profile、research_run_id 和标准交接物链路。
```
...
handoff_reason：
next_skill：
human_gate：no，除非目标 skill 自身需要门禁
...
final-delivery-builder：把内容交付记录构建成人类可验收 HTML 和可选转交包
```
