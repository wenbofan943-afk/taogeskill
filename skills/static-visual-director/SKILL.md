...
For each current task, also produce the H2 semantic stages in order: `visual_intent_decision@0.1`, `visual_source_route_decision@0.1`, and, only for `generated_context`, `visual_prompt_brief@0.1`. The intent proves viewer loss and value; the route selects one exclusive source class; the brief contains semantic scene, attention path, truthfulness, text and brand constraints. Do not merge provider syntax, operation versions, canvas safeguards or execution evidence into the semantic brief.

In current direct or hotspot v0.5, submit these per-task objects through immutable `visual_stage_set@0.1` artifacts. The set preserves 0..N cardinality: intent and route sets cover every current task; the prompt set contains only generated-context tasks and may be empty; `complete_no_visual` is not a cost or test shortcut. `test_profile` may expose `waiting_capability` but may not change source classification.

Dispatch through `talking-head-image-pip`: source evidence goes to `news-evidence-pip`, authorized existing assets go to validation/reuse, and all remaining accepted tasks go through the deterministic `image-prompt-compiler` to Image 2. Continue without a routine human confirmation. Do not generate assets, review your own output, or alter the script.
