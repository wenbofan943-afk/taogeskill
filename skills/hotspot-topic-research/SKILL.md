...
不负责写完整文案、不负责质检、不负责发布。默认允许把研究过程写入 `热点候选池.md`、`热点评分表.md` 和 `自媒体选题库.md`；这些只属于传播研究资产，不是产品规格、工程任务或发布承诺。
...
4. 如果档案不存在，按 账号档案完整性检查表.md 创建草案，并先问涛哥补齐 P0 字段。
...
补档案时必须按 `账号档案完整性检查表.md` 的口语化问法执行：
...
缺产品/活动对象边界时，不进入热点搜索；先按 [产品与活动对象档案](../../产品与活动对象档案.md) 补齐最小字段。
...
开始搜索前，必须先读 `热点搜索来源池.md`，按账号和三池生成本轮来源计划。
来源计划生成后，必须形成 `research_run_record`，并写入 `调研运行记录.md` 或在输出中给出可落盘记录。不要直接从“来源计划”跳到“热点候选池”。
...
```text
1. 本轮来源计划和实际搜索，写入 调研运行记录.md。
2. 本轮搜回来的热点，写入 热点候选池.md。
3. 完成评分后，评分摘要写入 热点评分表.md。
4. 只有通过桥接链、评分和策略判断的选题，才写入 自媒体选题库.md。
...
内容形式必须从 `内容形式类型与载体字典.md` 的内容形式允许值中选择。不要把案例拆解、产品演示、短视频画中画、直播切片当成一级内容形式。
