...
> 状态：alpha11_release_instruction
> 适用：GitHub 公开 alpha 预发行包或线下测试包。  
> 边界：本项目是内容工作流 skill 集，不是平台发布工具。
> Alpha 预发行提醒：`0.1.0-alpha.11` 是 GitHub 预发行版本；它不包含生产 runner，也不代表真实账号生产效果已验收。请先跑 `examples/` 和只读 checker，再决定是否用于私有账号试验。请勿使用已撤回的 Alpha.10。
...
1. 只从明确标记 `v0.1.0-alpha.11` 的可信 Release 取得 `taoge-creative-workflow-0.1.0-alpha.11-public-release.zip` 和同名 `.sha256`。
