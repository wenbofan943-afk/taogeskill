...
architecture_migration：ARCH-20260718-002_M6_runtime_conformance_suite_compiled_not_certified
架构说明：M1 静态编译 8/8、M2 direct shadow 16/16、M3 hotspot shadow 21/21、M4 session entry 19/19 继续通过。M5 已把 legacy blueprint/node 投影从 current IR 和 component catalog 移除，新建 `kernel_v1_current` session 改用 v0.2 binding 且不再绑定 compatibility catalog 摘要；旧 v0.1 binding 与 12 条 version-pinned R7 blueprint 只通过 `WorkflowCompatibilityLoader.ps1` 只读恢复。M5.1 又把 15 项 legacy 数据合同和 2 个实现文件迁入 `compatibility/legacy-r7/`，原 runtime/CLI 路径保留 2 个稳定 shim。M6-H1 evaluator suite 为 20/20 断言、2/2 负例；上一轮 clean HEAD evaluator certification 已通过。M6-H2 现已编译 runtime conformance，统一 M4/M2/M3 为 start/advance/wait/resume/rebuild/reconcile 六类命令，compile smoke 为 20/20 断言、4/4 负例、前后摘要一致。runtime suite 接入改变冻结集合，因此旧 evaluator 证书仅保留审计，下一步必须在新 clean HEAD 上重跑同摘要 evaluator/runtime certification；未联网、未调用 provider、未读取私有账号，项目保持 L2.8。
...
M6. M5/M5.1 compatibility isolation 与目录归档已完成。M6-H1 evaluator conformance 已编译并曾在 clean HEAD `30be209…` 上认证；M6-H2 已把 M4 session entry、M2 direct 和 M3 hotspot 收口为 runtime conformance，compile smoke 20/20、负例 4/4、六类命令全通过。由于 H2 扩大冻结集合，旧 evaluator 证书已登记为 `superseded_pending_same_digest_recertification`。下一步是在 H2 提交后的同一 clean HEAD 依次重跑 evaluator 与 runtime certification；direct/hotspot 真实 route 同摘要认证仍为后续。本批不等于 L3。
