...
3. R5-H6 已完成脱敏编译和回归；下一步是经用户授权只在私有生产区为当前账号做一次显式绑定迁移，再执行真实 H6 启动回归。迁移前 v0.2 正确阻断该旧账号；仍不自动采集、登录或发布。
