...
状态说明：R1-R4 既有范围和 P0-H1 至 H7 已完成产品 / Skill 编译；`0.1.0-alpha.6` 已作为 GitHub alpha prerelease 发布。当前仍是 alpha、单篇 runtime，不是生产级自动化 runner；PowerShell 7 旧记录不构成当前公开支持。R5-H1 至 H6 与 R6 / R3 当前合同已完成本地编译。上一轮私有真实稿件回归的 renderer 与响应式页面通过，但语义编排和 candidate 组装仍为高 Agent assist；R7-C01 至 C28 已确认，R7-H1 合同、H2 coordinator / submitter 与 H3 直供 producer adapters 已本地编译。12 个节点已绑定 payload Schema，submission envelope 由工具派生，业务状态到 payload 原生状态有显式映射，等待不写 current、不越 cursor。H4-H6 candidate / viewport / 热点与真实回归尚待本轮继续。alpha.6 尚未包含 R6 / R7 本地能力，未授权推送或发布。
...
2. R7-C01 至 C28 已确认；H1/H2 已完成合同与状态提交，H3 已完成 12 个直供 producer adapters、deterministic submission builder、业务状态映射和等待语义。下一批为 R7-H4 candidate v0.6 与 renderer；当前仍不声称 v0.6 HTML、viewport 或全链自主完成。
