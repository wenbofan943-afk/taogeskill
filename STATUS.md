...
3. R5 下一步是经用户授权后，用真实私有账号做一次人工辅助启动检查回归：只验证账号策略、最小补问和词库效果，不自动采集、登录或发布；H1 至 H5 均不创建真实品牌资产或外部热点采集实现。
