...
2. P0-H1 合同基础层与 P0-H2 compiler / renderer 已编译：5 个 H1 v0.2 Schema、H2 render receipt Schema、统一卡片输入、readiness derivation、delivery-first HTML、安全 / 结构检查及脱敏单篇 fixture 已落地。H2 专项 checker 共 16 项通过；下一批 P0-F03 至 F19 独立 runtime fixture 属于 H3。
...
13. P0-H2 已完成代码编译并通过 typed input、readiness、HTML、receipt、跨目录确定性、幂等复用和 v0.1 兼容检查，状态为 `p0_h2_compiled_ready_for_local_commit`；下一批为 P0-H3 独立失败 / 恢复 fixture。H6 新图片回归仍须在 H5 通过后另过图片授权与人工视觉门禁。
