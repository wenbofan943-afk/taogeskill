...
workflow_usage_state：v0.1.0-alpha.11_release_candidate_validating
...
状态说明：`0.1.0-alpha.10` 已在 GitHub 保留 tag 与 Release 作为可追溯记录，但因 GitHub source archive 含有被跟踪的第三方调研资料，已明确标记为撤回、不可使用。`0.1.0-alpha.11` 是后续补救候选：保留 Alpha.9 热点同 session 交付验收器、公开 fixture 与 visual-semantic helper 的闭包修复，同时把 `外部资料/` 改为 ignored 本地缓存并在 release gate/CI 中禁止受限根目录进入公开源码。Alpha.11 必须在完整本地候选、解包 ZIP 直供 / 热点验收、远端 exact-commit Actions 后才能写为已发布。R8-C01 至 C70 已确认；H1-H4 与 H5R1-H5R5 已完成本地编译和本轮确定性评估收口，业务 Skill inventory 为 28 个。`hotspot-topic-research` 已从 953 行降到 150 行，`propagation-router` 已从 777 行降到 70 行，`platform-packaging-adapter` 已从 665 行降到 56 行。H5 v0.2 已编译 typed input、独立双臂、机器审计、匿名包、human verdict recorder、唯一 finalizer 与 finalization-only state projection；整项目仍保持 L2.8。
...
Git：已初始化独立本地工作母仓，当前分支 `main`；无凭据 HTTPS 远端为 `https://github.com/wenbofan943-afk/taogeskill.git`；上一可用发布 tag 为 `v0.1.0-alpha.9`，`v0.1.0-alpha.10` 已撤回但保留可追溯，当前候选为 `v0.1.0-alpha.11`；Git 入口由执行环境解析为 `<GIT_EXE>`
```
...
产品真源见 [产品文档索引的当前产品优先级检查点](docs/product/README.md#当前产品优先级检查点)：本项目先以直供创作、热点创作两条主路径做少量真实账号 / 真实选题 / 真实交付的重复验证；人保留选题与最终发布判断。没有真实运行直接阻断时，公开 / 兼容链路冻结，不新增平台化、自动发布或泛化 SaaS 建设。未来仅作为“短视频创作项目”的可验证内容生产引擎能力槽位，不实施接入。
