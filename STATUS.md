...
architecture_migration：ARCH-20260718-002_M6_evaluator_conformance_suite_compiled_not_certified
架构说明：M1 静态编译 8/8、M2 direct shadow 16/16、M3 hotspot shadow 21/21、M4 session entry 19/19 继续通过。M5 已把 legacy blueprint/node 投影从 current IR 和 component catalog 移除，新建 `kernel_v1_current` session 改用 v0.2 binding 且不再绑定 compatibility catalog 摘要；旧 v0.1 binding 与 12 条 version-pinned R7 blueprint 只通过 `WorkflowCompatibilityLoader.ps1` 只读恢复。M5.1 又把 15 项 legacy 数据合同和 2 个实现文件迁入 `compatibility/legacy-r7/`，原 runtime/CLI 路径保留 2 个稳定 shim。M6-H1 已编译产品/架构/runtime/evaluator/fixture 五类 freeze、隔离 work root 和 evaluator conformance suite；Windows PowerShell 5.1 compile smoke 为 20/20 断言、2/2 负例、前后摘要一致。该 smoke 绑定未提交 worktree，因此 evaluator/runtime certification 仍为 `not_run`；未联网、未调用 provider、未读取私有账号，项目保持 L2.8。
...
M6. M5/M5.1 compatibility isolation 与目录归档已完成。M6-H1 已编译五类 freeze manifest、隔离 evaluator work root、20 个 known-answer 断言和 2 个 freeze false-success 负例；compile smoke 通过但按合同保持 `not_run_compile_smoke_only`。下一步是单独执行 clean HEAD evaluator certification；通过后才能编译/认证 runtime start/advance/wait/resume/rebuild/reconcile，再执行同一 digest 绑定的 direct/hotspot 真实认证。当前 evaluator/runtime certification 均未运行，M6-H1 不等于 L3。
