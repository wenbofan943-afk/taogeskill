...
function Get-R7ViewportEvidenceRelativeRoot {
  param([int]$DeliveryRevision=1)
  if($DeliveryRevision-lt1){throw 'viewport_revision_invalid'}
  return "intermediate/visual-review/r7/r$DeliveryRevision"
}

function Invoke-R7ViewportProfile {
  param([string]$ProjectRoot,[string]$SessionRoot,[string]$HtmlPath,[string]$ProfileId,[int]$Width,[int]$Height,[object]$HostInfo,[switch]$OmitScreenshot,[int]$DeliveryRevision=1)
  $evidenceRoot=Join-Path $SessionRoot (Get-R7ViewportEvidenceRelativeRoot $DeliveryRevision);if(-not(Test-Path $evidenceRoot)){New-Item -ItemType Directory -Path $evidenceRoot -Force|Out-Null}
...
function Test-R7ViewportReportContract {
  param([object]$Report)
  $isH7=[string]$Report.schema_id-eq'taoge://schemas/r7/viewport-acceptance/v0.2';$errors=[Collections.Generic.List[string]]::new();$required=@('schema_id','schema_version','viewport_report_id','session_id','final_delivery_ref','html_path','html_sha256','browser_invocation_status','layout_measurement_status','overall_result','profiles','artifact_execution_contribution','workflow_autonomous_completion_count','autonomy_eligible','warning_codes','created_at','next_skill')+$(if($isH7){@('technical_viewport_status')}else{@('visual_acceptance_status')})
  ...
  if($errors.Count){return [object[]]$errors.ToArray()};if(-not(($isH7-and[string]$Report.schema_version-eq'0.2')-or($Report.schema_id-eq'taoge://schemas/r7/viewport-acceptance/v0.1'-and[string]$Report.schema_version-eq'0.1'))){$errors.Add('viewport_report_version_invalid')};if(-not(Test-P0Digest $Report.html_sha256)){$errors.Add('viewport_html_digest_invalid')}
  if($isH7-and(Test-R7HasProperty $Report 'visual_acceptance_status')){$errors.Add('viewport_visual_status_forbidden')}
...
function Invoke-R7ViewportAcceptance {
  ...
  try{$deliveryCurrent=Get-R7CandidateCurrentArtifact $sessionRoot 'final_delivery';$delivery=$deliveryCurrent.Payload;$plan=Read-P0JsonFile (Join-Path $sessionRoot 'intermediate/p0/session-execution-plan.json');$isH7=[string]$plan.plan_schema_id-in@('taoge://schemas/p0/session-execution-plan/v1.0','taoge://schemas/p0/session-execution-plan/v1.1','taoge://schemas/p0/session-execution-plan/v1.2');$viewportRevision=$(if([string]$plan.plan_schema_id-in@('taoge://schemas/p0/session-execution-plan/v0.9','taoge://schemas/p0/session-execution-plan/v1.0','taoge://schemas/p0/session-execution-plan/v1.1','taoge://schemas/p0/session-execution-plan/v1.2')){[int]$plan.plan_revision}else{1});$htmlPath=Resolve-R7RuntimePath $sessionRoot ([string]$delivery.html_path);if(-not(Test-Path -LiteralPath $htmlPath)){throw 'final_html_missing'};$htmlDigest=Get-R7RuntimeHash $htmlPath;if($htmlDigest-ne[string]$delivery.html_sha256){throw 'final_html_digest_mismatch'}
    ...
    else{$runtimeErrors=[Collections.Generic.List[string]]::new();foreach($definition in @([pscustomobject]@{Id='desktop-1440x1000';Width=1440;Height=1000},[pscustomobject]@{Id='mobile-390x844';Width=390;Height=844})){$result=Invoke-R7ViewportProfile $ProjectRoot $sessionRoot $htmlPath $definition.Id $definition.Width $definition.Height $hostInfo -DeliveryRevision $viewportRevision;if($result.ResultCode-ne'pass'){$overall=[string]$result.ResultCode;$visualStatus=if($result.ResultCode-eq'visual_acceptance_fail'){'visual_acceptance_fail'}else{'not_tested'};foreach($error in @($result.Errors)){$runtimeErrors.Add([string]$error)};if($result.ResultCode-eq'evidence_pending'){$browserStatus='evidence_pending'}elseif($result.ResultCode-eq'browser_invocation_error'){$browserStatus='error';$layoutStatus='error'};break};$profiles.Add($result.Profile)}}
    if($overall-in@('visual_acceptance_fail','browser_invocation_error','evidence_pending')){if($runtimeErrors.Count-eq0){$runtimeErrors.Add($overall)};$resultCode=$(if($overall-eq'evidence_pending'){'browser_invocation_error'}elseif($isH7-and$overall-eq'visual_acceptance_fail'){'technical_viewport_fail'}else{$overall});return New-R7RuntimeResult $resultCode 2 ([pscustomobject]@{Profiles=[object[]]$profiles.ToArray();BrowserStatus=$browserStatus}) $runtimeErrors.ToArray()}
    $autonomy=Get-R7AutonomyEvidence $sessionRoot $overall;if($isH7){$report=[ordered]@{schema_id='taoge://schemas/r7/viewport-acceptance/v0.2';schema_version='0.2';viewport_report_id="VIEWPORT-$(Split-Path -Leaf $sessionRoot)-$('{0:000}'-f$viewportRevision)";session_id=Split-Path -Leaf $sessionRoot;final_delivery_ref=[ordered]@{artifact_id=[string]$delivery.final_delivery_id;sha256=[string]$deliveryCurrent.Sha256};html_path=[string]$delivery.html_path;html_sha256=$htmlDigest;browser_invocation_status=$browserStatus;layout_measurement_status=$layoutStatus;technical_viewport_status=$(if($overall-eq'pass'){'pass'}else{'not_tested'});overall_result=$overall;profiles=[object[]]$profiles.ToArray();artifact_execution_contribution=$autonomy.Contribution;workflow_autonomous_completion_count=[int]$autonomy.WorkflowAutonomousCompletionCount;autonomy_eligible=[bool]$autonomy.AutonomyEligible;warning_codes=[object[]]$warnings.ToArray();created_at=[DateTimeOffset]::UtcNow.ToString('o');next_skill='business-delivery-acceptance'}}else{$report=[ordered]@{schema_id='taoge://schemas/r7/viewport-acceptance/v0.1';schema_version='0.1';viewport_report_id="VIEWPORT-$(Split-Path -Leaf $sessionRoot)-$('{0:000}'-f$viewportRevision)";session_id=Split-Path -Leaf $sessionRoot;final_delivery_ref=[ordered]@{artifact_id=[string]$delivery.final_delivery_id;sha256=[string]$deliveryCurrent.Sha256};html_path=[string]$delivery.html_path;html_sha256=$htmlDigest;browser_invocation_status=$browserStatus;layout_measurement_status=$layoutStatus;visual_acceptance_status=$visualStatus;overall_result=$overall;profiles=[object[]]$profiles.ToArray();artifact_execution_contribution=$autonomy.Contribution;workflow_autonomous_completion_count=[int]$autonomy.WorkflowAutonomousCompletionCount;autonomy_eligible=[bool]$autonomy.AutonomyEligible;warning_codes=[object[]]$warnings.ToArray();created_at=[DateTimeOffset]::UtcNow.ToString('o');next_skill='propagation-router'}}
  ...
  $commit=Commit-R7DeterministicArtifact $ProjectRoot $sessionRoot $(if($isH7){'viewport_acceptance_h7'}else{'viewport_acceptance'}) 'viewport_acceptance_report' ([string]$reportObject.viewport_report_id) $reportObject ([string]$reportObject.overall_result) @([string]$delivery.final_delivery_id) $(if($isH7){@('R7-C120','R7-C121','R7-C122')}else{@('R7-F14','R7-F17','R7-F19','R7-F20')})
  if($commit.ExitCode-eq0-and[string]$plan.plan_schema_id-in@('taoge://schemas/p0/session-execution-plan/v0.9','taoge://schemas/p0/session-execution-plan/v1.1','taoge://schemas/p0/session-execution-plan/v1.2')-and[int]$plan.plan_revision-gt1){if(-not(Get-Command Complete-R7ActiveRevisionRequest -ErrorAction SilentlyContinue)){. (Join-Path $PSScriptRoot 'R7HumanRevisionRuntime.ps1')};$closed=Complete-R7ActiveRevisionRequest $sessionRoot $plan;if($closed.ExitCode-ne0){return $closed}}
  return $commit
}
...
function Get-R7FinalHumanSubmissionIdentity {
  param([string]$SessionId,[string]$DecisionStatus,[bool]$IsV02,[int]$RecordRevision)
  $revision=$(if($IsV02){$RecordRevision}else{1})
  $id=$(if($IsV02){"SUB-$SessionId-final_human_gate-$DecisionStatus-r$revision"}else{"SUB-$SessionId-final_human_gate-$DecisionStatus"})
  return [pscustomobject]@{SubmissionId=$id;OutputRevision=$revision}
}

function Test-R7WorkflowSessionRecordContract {
  param([object]$Payload)
  $errors=[Collections.Generic.List[string]]::new();$required=@('schema_id','schema_version','session_record_id','session_id','final_delivery_ref','viewport_report_ref','decision_status','requested_action','target_artifact_ref','decided_by','decided_at','next_skill');$isV03=[string]$Payload.schema_id-eq'taoge://schemas/r7/workflow-session-record/v0.3';$isV02=[string]$Payload.schema_id-eq'taoge://schemas/r7/workflow-session-record/v0.2';if($isV02-or$isV03){$required+=@('delivery_revision_request_ref')};if($isV03){$required+=@('business_delivery_acceptance_ref')}
  ...
  if(-not(($Payload.schema_id-eq'taoge://schemas/r7/workflow-session-record/v0.1'-and[string]$Payload.schema_version-eq'0.1')-or($isV02-and[string]$Payload.schema_version-eq'0.2')-or($isV03-and[string]$Payload.schema_version-eq'0.3'))){$errors.Add('workflow_session_record_version_invalid')}
...
function New-R7FinalHumanSubmission {
  ...
  $sessionRoot=[IO.Path]::GetFullPath($Session);$task=Read-R7JsonFile (Resolve-R7RuntimePath $sessionRoot "intermediate/r7/tasks/$TaskEnvelopeId.json");if([string]$task.node_id-notin@('final_human_gate','final_human_gate_h7')){return New-R7RuntimeResult 'final_human_task_invalid' 1 $task @()}
  if([string]$task.blueprint_version-in@('0.3','0.4','0.5')-and$DecisionStatus-eq'revision_requested'){return New-R7RuntimeResult 'delivery_revision_request_required' 1 $task @('use Invoke-R7HumanRevisionRequest with change_items')}
  ...
  $delivery=Get-R7CandidateCurrentArtifact $sessionRoot 'final_delivery';$viewport=Get-R7CandidateCurrentArtifact $sessionRoot 'viewport_acceptance_report';$isH7=[string]$task.blueprint_version-in@('0.4','0.5');$business=$(if($isH7){Get-R7CandidateCurrentArtifact $sessionRoot 'business_delivery_acceptance'}else{$null});if($isH7-and$DecisionStatus-in@('human_confirm','export_requested')-and[string]$business.Payload.business_delivery_status-eq'business_delivery_rejected'){return New-R7RuntimeResult 'business_delivery_acceptance_blocked' 1 $business.Payload @()};$targetRef=$null
  ...
  $sessionId=Split-Path -Leaf $sessionRoot;$isV02=[string]$task.blueprint_version-in@('0.3','0.4','0.5');$plan=Read-P0JsonFile (Join-Path $sessionRoot 'intermediate/p0/session-execution-plan.json');$recordRevision=$(if($isV02){[int]$plan.plan_revision}else{1});$payload=[ordered]@{schema_id=$(if($isH7){'taoge://schemas/r7/workflow-session-record/v0.3'}elseif($isV02){'taoge://schemas/r7/workflow-session-record/v0.2'}else{'taoge://schemas/r7/workflow-session-record/v0.1'});schema_version=$(if($isH7){'0.3'}elseif($isV02){'0.2'}else{'0.1'});session_record_id="SESSIONREC-$sessionId-$('{0:000}'-f$recordRevision)";session_id=$sessionId;final_delivery_ref=[ordered]@{artifact_id=[string]$delivery.Pointer.artifact_id;sha256=[string]$delivery.Sha256};viewport_report_ref=[ordered]@{artifact_id=[string]$viewport.Pointer.artifact_id;sha256=[string]$viewport.Sha256};decision_status=$DecisionStatus;requested_action=$RequestedAction;target_artifact_ref=$targetRef;decided_by='human';decided_at=[DateTimeOffset]::UtcNow.ToString('o');next_skill=$(switch($DecisionStatus){human_confirm{'done'}revision_requested{'semantic-workflow-coordinator'}export_requested{'handoff-exporter'}archive_requested{'archive-session'}})}
  if($isV02){$payload.delivery_revision_request_ref=$null};if($isH7){$payload.business_delivery_acceptance_ref=[ordered]@{artifact_id=[string]$business.Pointer.artifact_id;sha256=[string]$business.Sha256}}
  ...
  $submissionIdentity=Get-R7FinalHumanSubmissionIdentity $sessionId $DecisionStatus $isV02 $recordRevision;$submissionId=[string]$submissionIdentity.SubmissionId;$submission=[ordered]@{schema_id='taoge://schemas/r7/semantic-artifact-submission/v0.2';schema_version='0.2';submission_id=$submissionId;task_envelope_id=[string]$task.task_envelope_id;session_id=$sessionId;plan_id=[string]$task.plan_id;node_id=[string]$task.node_id;skill_ref=[string]$task.skill_ref;attempt_no=1;submitted_at=[DateTimeOffset]::UtcNow.ToString('o');input_binding_digest=[string]$task.input_binding_digest;output_artifact_type='workflow_session_record';output_contract_version=[string]$task.task_contract_version;output_artifact_id=[string]$payloadObject.session_record_id;output_revision=[int]$submissionIdentity.OutputRevision;result_status=$DecisionStatus;requested_action=$RequestedAction;source_artifact_ids=@($task.input_artifact_bindings|ForEach-Object{[string]$_.artifact_id});quality_status='pass';delivery_eligibility='trace_only';check_ids=@('R7-H7-FINAL-HUMAN-GATE');payload=$payloadObject;evidence_refs=@();idempotency_key=[string]$task.idempotency_key;write_intent='submit_for_deterministic_commit';requested_machine_writes=@()}
