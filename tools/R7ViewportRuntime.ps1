...
function Invoke-R7ViewportAcceptance {
  ...
  try{$deliveryCurrent=Get-R7CandidateCurrentArtifact $sessionRoot 'final_delivery';$delivery=$deliveryCurrent.Payload;$htmlPath=Resolve-R7RuntimePath $sessionRoot ([string]$delivery.html_path);if(-not(Test-Path -LiteralPath $htmlPath)){throw 'final_html_missing'};$htmlDigest=Get-R7RuntimeHash $htmlPath;if($htmlDigest-ne[string]$delivery.html_sha256){throw 'final_html_digest_mismatch'}
  ...
  return Commit-R7DeterministicArtifact $ProjectRoot $sessionRoot 'viewport_acceptance' 'viewport_acceptance_report' ([string]$reportObject.viewport_report_id) $reportObject ([string]$reportObject.overall_result) @([string]$delivery.final_delivery_id) @('R7-F14','R7-F17','R7-F19','R7-F20')
}
...
function Test-R7WorkflowSessionRecordContract {
  ...
  $errors=[Collections.Generic.List[string]]::new();$required=@('schema_id','schema_version','session_record_id','session_id','final_delivery_ref','viewport_report_ref','decision_status','requested_action','target_artifact_ref','decided_by','decided_at','next_skill')
  ...
  if($Payload.schema_id-ne'taoge://schemas/r7/workflow-session-record/v0.1'-or[string]$Payload.schema_version-ne'0.1'){$errors.Add('workflow_session_record_version_invalid')}
  if([string]$Payload.requested_action -notin @(Get-R7FinalDecisionAllowedActions ([string]$Payload.decision_status))){$errors.Add('decision_action_mismatch')}
...
function New-R7FinalHumanSubmission {
  ...
  $sessionId=Split-Path -Leaf $sessionRoot;$payload=[ordered]@{schema_id='taoge://schemas/r7/workflow-session-record/v0.1';schema_version='0.1';session_record_id="SESSIONREC-$sessionId-001";session_id=$sessionId;final_delivery_ref=[ordered]@{artifact_id=[string]$delivery.Pointer.artifact_id;sha256=[string]$delivery.Sha256};viewport_report_ref=[ordered]@{artifact_id=[string]$viewport.Pointer.artifact_id;sha256=[string]$viewport.Sha256};decision_status=$DecisionStatus;requested_action=$RequestedAction;target_artifact_ref=$targetRef;decided_by='human';decided_at=[DateTimeOffset]::UtcNow.ToString('o');next_skill=$(switch($DecisionStatus){human_confirm{'done'}revision_requested{'owning_producer'}export_requested{'handoff-exporter'}archive_requested{'archive-session'}})}
