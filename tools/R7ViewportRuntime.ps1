...
function New-R7FinalHumanSubmission {
  ...
  if([string]$task.blueprint_version-in@('0.3','0.4')-and$DecisionStatus-eq'revision_requested'){return New-R7RuntimeResult 'delivery_revision_request_required' 1 $task @('use Invoke-R7HumanRevisionRequest with change_items')}
  ...
  $delivery=Get-R7CandidateCurrentArtifact $sessionRoot 'final_delivery';$viewport=Get-R7CandidateCurrentArtifact $sessionRoot 'viewport_acceptance_report';$isH7=[string]$task.blueprint_version-eq'0.4';$business=$(if($isH7){Get-R7CandidateCurrentArtifact $sessionRoot 'business_delivery_acceptance'}else{$null});if($isH7-and$DecisionStatus-in@('human_confirm','export_requested')-and[string]$business.Payload.business_delivery_status-eq'business_delivery_rejected'){return New-R7RuntimeResult 'business_delivery_acceptance_blocked' 1 $business.Payload @()};$targetRef=$null
