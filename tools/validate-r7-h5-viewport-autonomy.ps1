...
try{
  ...
  $oldNodePath=$env:NODE_PATH;$env:NODE_PATH=$partialModuleRoot;try{$hostInfo=Get-R7ViewportHost}finally{$env:NODE_PATH=$oldNodePath}
  if(-not$hostInfo.Available){throw 'complete_viewport_dependency_candidate_not_selected_after_partial_candidate'}
