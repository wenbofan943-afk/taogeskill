...
try{
  ...
  $renderedHtmlPath=Join-Path $valid 'deliverables/final-delivery.html'
  if($compile.ExitCode-ne0-or$render.ExitCode-ne0-or-not(Test-Path -LiteralPath $renderedHtmlPath)){throw("h7_compile_render_failed:compile_exit=$($compile.ExitCode);compile=$($compile.Text);render_exit=$($render.ExitCode);render=$($render.Text);html_exists=$(Test-Path -LiteralPath $renderedHtmlPath)")}
  ...
  $html=Get-Content $renderedHtmlPath -Raw -Encoding UTF8
