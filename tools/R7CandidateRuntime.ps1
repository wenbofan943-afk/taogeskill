...
function New-R7CandidatePayload {
  ...
  $presentation=if($isHotspot-or$isV08){Read-YamlFile (Join-Path $ProjectRoot 'routes/r7-delivery-presentation-registry.yaml')}else{Read-WorkflowCompatibilityYamlAsset -ProjectRoot $ProjectRoot -AssetReference 'compatibility/legacy-r7/routes/r7-delivery-presentation-registry.v0.1.yaml' -CallerRuntimeGeneration 'legacy_r7'}
  $actions=if($isV09){Read-YamlFile (Join-Path $ProjectRoot 'routes/r7-action-registry.yaml')}elseif($isV08-or$isHotspot){Read-WorkflowCompatibilityYamlAsset -ProjectRoot $ProjectRoot -AssetReference 'compatibility/legacy-r7/routes/r7-action-registry.v0.3.yaml' -CallerRuntimeGeneration 'legacy_r7'}else{Read-WorkflowCompatibilityYamlAsset -ProjectRoot $ProjectRoot -AssetReference 'compatibility/legacy-r7/routes/r7-action-registry.v0.1.yaml' -CallerRuntimeGeneration 'legacy_r7'}
  ...
  if($isV09){
    if([string]::IsNullOrWhiteSpace([string]$platformPackage.delivery_title)){throw 'candidate_delivery_title_missing'}
    if([string]$deliveryAssetSet.image_asset_set_ref.artifact_id-ne[string]$assetSet.image_asset_set_id-or[string]$deliveryAssetSet.image_asset_set_ref.sha256-ne[string]$s.image_asset_set.Sha256){throw 'candidate_delivery_asset_set_binding_mismatch'}
    if([string]$deliveryAssetSet.finalize_status-notin@('finalized','finalized_with_warnings','finalized_no_visual')){throw 'candidate_delivery_asset_set_not_finalized'}
  }
  ...
  $isAssetSetV03=[string]$assetSet.schema_id-eq'taoge://schemas/r7/image-asset-set/v0.3';$isAssetSetV04=[string]$assetSet.schema_id-eq'taoge://schemas/r7/image-asset-set/v0.4';$isAssetSetV02=[string]$assetSet.schema_id-in@('taoge://schemas/r7/image-asset-set/v0.2','taoge://schemas/r7/image-asset-set/v0.3','taoge://schemas/r7/image-asset-set/v0.4');$usesDeliveryAssetSet=$isAssetSetV03-or$isAssetSetV04
  $assetById=@{};foreach($candidateAsset in @($assetSet.assets)){$assetById[[string]$candidateAsset.asset_id]=$candidateAsset}
  $assetsForDelivery=if($usesDeliveryAssetSet){@($deliveryAssetSet.delivery_assets|ForEach-Object{$assetById[[string]$_.delivery_asset_ref.asset_id]})}else{@($assetSet.assets)}
  $assetByTask=@{};$providerAttemptsByTask=@{};$captureAttemptsByTask=@{};foreach($asset in @($assetsForDelivery)){
  ...
  $assetSetClaimsReady=[string]$assetSet.asset_set_status -in @('materialized','ready_with_warnings','materialized_waiting_finalize','ready_with_warnings_waiting_finalize','materialized_waiting_review','ready_with_warnings_waiting_review')
  ...
  $visualDeliveryReadiness=switch([string]$assetSet.asset_set_status){
    ...
    'materialized_waiting_finalize'{$(if($isV09-and$deliveryAssetSet.finalize_status-eq'finalized'){'ready'}else{'waiting_assets'})}
    'ready_with_warnings_waiting_finalize'{$(if($isV09-and$deliveryAssetSet.finalize_status-in@('finalized','finalized_with_warnings')){'ready_with_warnings'}else{'waiting_assets'})}
    'materialized_waiting_review'{$(if($isV09-and$deliveryAssetSet.finalize_status-in@('finalized','finalized_with_warnings')){'ready'}else{'waiting_assets'})}
    'ready_with_warnings_waiting_review'{$(if($isV09-and$deliveryAssetSet.finalize_status-in@('finalized','finalized_with_warnings')){'ready_with_warnings'}else{'waiting_assets'})}
    'no_visual_waiting_review'{$(if($isV09-and$deliveryAssetSet.finalize_status-eq'finalized_no_visual'){'ready'}else{'waiting_assets'})}
  ...
  $deliveryRevisionNo=$(if($isV08){[int]$plan.plan_revision}else{1});$finalDeliveryId="FD-$sessionId-$('{0:000}'-f$deliveryRevisionNo)";$revisionId="DREV-$sessionId-$('{0:000}'-f$deliveryRevisionNo)"
  ...
  $supersedesDeliveryRef=$null
  if($isV08){
    $innerObject.delivery_revision.revision_no=$deliveryRevisionNo
    if($deliveryRevisionNo-gt1){
      $revisionFolder="deliverables/revisions/r$deliveryRevisionNo"
      $innerObject.delivery_revision.generated_view_paths.final_html="$revisionFolder/final-delivery.html";$innerObject.delivery_revision.generated_view_paths.final_script="$revisionFolder/final-script.md";$innerObject.delivery_revision.generated_view_paths.final_visual_plan="$revisionFolder/final-visual-plan.md";$innerObject.delivery_revision.generated_view_paths.final_platform_package="$revisionFolder/final-platform-package.md";$innerObject.delivery_revision.generated_view_paths.content_delivery_record="$revisionFolder/content-delivery-record.md";$innerObject.delivery_revision.generated_view_paths.revision_manifest="$revisionFolder/delivery-revision.json"
      $oldDeliveryPointer=Resolve-R7RuntimePath $SessionRoot 'intermediate/r7/current/final_delivery.json';if(Test-Path -LiteralPath $oldDeliveryPointer){$oldPointer=Read-R7JsonFile $oldDeliveryPointer;$supersedesDeliveryRef=[ordered]@{artifact_id=[string]$oldPointer.artifact_id;sha256=[string]$oldPointer.sha256}}
    }
  }
  ...
  $actionRegistryRef=$(if($isV09){'routes/r7-action-registry.yaml'}elseif($isV08-or$isHotspot){'compatibility/legacy-r7/routes/r7-action-registry.v0.3.yaml'}else{'compatibility/legacy-r7/routes/r7-action-registry.v0.1.yaml'});$presentationRegistryRef=$(if($isHotspot-or$isV08){'routes/r7-delivery-presentation-registry.yaml'}else{'compatibility/legacy-r7/routes/r7-delivery-presentation-registry.v0.1.yaml'})
  $actionRegistryPath=$(if($isV09){Join-Path $ProjectRoot $actionRegistryRef}else{Resolve-WorkflowCompatibilityAsset -ProjectRoot $ProjectRoot -AssetReference $actionRegistryRef -CallerRuntimeGeneration 'legacy_r7'});$presentationRegistryPath=$(if($isHotspot-or$isV08){Join-Path $ProjectRoot $presentationRegistryRef}else{Resolve-WorkflowCompatibilityAsset -ProjectRoot $ProjectRoot -AssetReference $presentationRegistryRef -CallerRuntimeGeneration 'legacy_r7'})
  $registryMap=@([ordered]@{path=$actionRegistryRef;sha256=Get-R7RuntimeHash $actionRegistryPath},[ordered]@{path=$presentationRegistryRef;sha256=Get-R7RuntimeHash $presentationRegistryPath})
  ...
  if($isV08){
    $visualRoutes=[Collections.Generic.List[object]]::new();foreach($asset in @($assetsForDelivery)){$binding=$(if($isV09){@($deliveryAssetSet.delivery_assets|Where-Object{$_.visual_task_id-eq$asset.visual_task_id})|Select-Object -First 1}else{$null});$base=$(if($isV09){$candidateBase=$assetById[[string]$binding.base_asset_ref.asset_id];if($null-ne$candidateBase){$candidateBase}else{$asset}}else{$asset});$visualRoutes.Add([ordered]@{visual_task_id=[string]$asset.visual_task_id;source_class=$(if(Test-R7HasProperty $asset 'source_class'){[string]$asset.source_class}else{'historical_unspecified'});route_decision=$(if(Test-R7HasProperty $asset 'route_decision'){[string]$asset.route_decision}elseif(Test-R7HasProperty $asset 'source_mode'){[string]$asset.source_mode}else{'historical_unspecified'});base_asset_ref=$(if($isV09){$binding.base_asset_ref}elseif(Test-R7HasProperty $asset 'base_asset_ref'){$asset.base_asset_ref}else{$null});delivery_asset_ref=$(if($isV09){$binding.delivery_asset_ref}else{$null});parent_sha256=$(if($isV09-and$null-ne$asset.parent_asset_ref){[string]$asset.parent_asset_ref.sha256}elseif(Test-R7HasProperty $asset 'parent_sha256'){$asset.parent_sha256}else{$null});provider_or_capture=$(if(Test-R7HasProperty $base 'provider_or_capture'){[string]$base.provider_or_capture}elseif((Test-R7HasProperty $base 'provider_invoked') -and [bool]$base.provider_invoked){'image2'}else{'authorized_existing_asset'});postprocess=$(if(Test-R7HasProperty $asset 'postprocess'){$asset.postprocess}else{[ordered]@{mode='historical'}});visual_review_ref=$(if($isV09){$binding.visual_review_ref}else{$null});reuse_authorization_ref=$(if(Test-R7HasProperty $base 'reuse_authorization_ref'){$base.reuse_authorization_ref}else{$null});evidence_parity_result=$(if(Test-R7HasProperty $base 'evidence_parity_result'){[string]$base.evidence_parity_result}else{'not_applicable'})})}
    ...
    if($isV09){$outer=[ordered]@{schema_id='taoge://schemas/final-delivery/typed-components/v0.9';schema_version='typed_components_v0.9';final_delivery_id=$finalDeliveryId;session_id=$sessionId;delivery_title=[string]$platformPackage.delivery_title;candidate_status=$(if($warnings.Count){'compiled_with_warnings'}else{'compiled'});action_registry_version='r7-action-registry-v0.2';presentation_registry_version='r7-delivery-presentation-registry-v0.2';compiler_provenance=[ordered]@{producer='deterministic_compiler';compiler_version='p0-deterministic-delivery-candidate-compiler-v0.9';compiled_at=[DateTimeOffset]::UtcNow.ToString('o')};content_source_context=$context;source_map=[object[]]$SourceSet.SourceMap;source_binding_digest=$bindingDigest;artifact_execution_contribution=$contribution;external_activity_counts=[ordered]@{network_read_count=$parsedNetwork;source_capture_attempt_count=$captureAttempts;image_provider_attempt_count=[int]$assetSet.provider_invocation_count;external_side_effect_step_completed_count=[int]$contribution.external_side_effect_step_completed_count};execution_transparency=[ordered]@{test_profile=[string]$plan.test_profile;image2_runtime_profile='not_observable';manual_patch_detected=$false;compatibility_normalization_applied=$false};visual_route_summary=[object[]]$visualRoutes.ToArray();delivery_asset_set_ref=Get-R7CandidateArtifactRef $s.image_asset_delivery_set;page_information_architecture=[ordered]@{primary_section_order=@('delivery_header','script','visual_inserts','platform_delivery','warnings','actions');audit_default_collapsed=$true};readiness_derivation=[ordered]@{platform_profile_status='provisional_profiles_disclosed';maximum_publish_readiness='ready_for_manual_publish';warning_codes=[object[]]$innerObject.production_status.warning_codes};revision_context=[ordered]@{delivery_revision_no=$deliveryRevisionNo;supersedes_delivery_ref=$supersedesDeliveryRef;active_revision_request_ref=$activeRevisionRef};delivery_payload=$innerObject}}
    else{$outer=[ordered]@{schema_id='taoge://schemas/final-delivery/typed-components/v0.8';schema_version='typed_components_v0.8';final_delivery_id=$finalDeliveryId;session_id=$sessionId;candidate_status=$(if($warnings.Count){'compiled_with_warnings'}else{'compiled'});action_registry_version='r7-action-registry-v0.2';presentation_registry_version='r7-delivery-presentation-registry-v0.2';compiler_provenance=[ordered]@{producer='deterministic_compiler';compiler_version='p0-deterministic-delivery-candidate-compiler-v0.8';compiled_at=[DateTimeOffset]::UtcNow.ToString('o')};content_source_context=$context;source_map=[object[]]$SourceSet.SourceMap;source_binding_digest=$bindingDigest;artifact_execution_contribution=$contribution;external_activity_counts=[ordered]@{network_read_count=$parsedNetwork;source_capture_attempt_count=$captureAttempts;image_provider_attempt_count=[int]$assetSet.provider_invocation_count;external_side_effect_step_completed_count=[int]$contribution.external_side_effect_step_completed_count};execution_transparency=[ordered]@{test_profile=[string]$plan.test_profile;image2_runtime_profile='not_observable';manual_patch_detected=$false};visual_route_summary=[object[]]$visualRoutes.ToArray();revision_context=[ordered]@{delivery_revision_no=$deliveryRevisionNo;supersedes_delivery_ref=$supersedesDeliveryRef;active_revision_request_ref=$activeRevisionRef};delivery_payload=$innerObject}}
  }
...
function Test-R7CandidateV09Contract {
  param([object]$Candidate,[object]$SourceSet=$null)
  $errors=[Collections.Generic.List[string]]::new();$required=@('schema_id','schema_version','final_delivery_id','session_id','delivery_title','candidate_status','action_registry_version','presentation_registry_version','compiler_provenance','content_source_context','source_map','source_binding_digest','artifact_execution_contribution','external_activity_counts','execution_transparency','visual_route_summary','delivery_asset_set_ref','page_information_architecture','readiness_derivation','revision_context','delivery_payload')
  foreach($error in(Test-P0RequiredProperties $Candidate $required 'candidate_v09')){$errors.Add($error)};foreach($error in(Test-P0AllowedProperties $Candidate $required 'candidate_v09')){$errors.Add($error)}
  if($errors.Count){return [object[]]$errors.ToArray()}
  if([string]$Candidate.schema_id-ne'taoge://schemas/final-delivery/typed-components/v0.9'-or[string]$Candidate.schema_version-ne'typed_components_v0.9'){$errors.Add('candidate_v09_version_invalid')}
  if([string]::IsNullOrWhiteSpace([string]$Candidate.delivery_title)){$errors.Add('candidate_v09_delivery_title_missing')}
  if([bool]$Candidate.execution_transparency.compatibility_normalization_applied){$errors.Add('candidate_v09_compatibility_normalization_forbidden')}
  $expected=@('delivery_header','script','visual_inserts','platform_delivery','warnings','actions');if([string]::Join('|',@($Candidate.page_information_architecture.primary_section_order))-ne[string]::Join('|',$expected)-or-not[bool]$Candidate.page_information_architecture.audit_default_collapsed){$errors.Add('candidate_v09_information_architecture_invalid')}
  foreach($route in @($Candidate.visual_route_summary)){if($null-eq$route.delivery_asset_ref-or$null-eq$route.visual_review_ref){$errors.Add("candidate_v09_final_asset_binding_missing:$($route.visual_task_id)")}}
  foreach($error in(Test-P0RenderInputV05Contract $Candidate.delivery_payload)){$errors.Add("candidate_v09_delivery_payload:$error")}
  return [object[]]$errors.ToArray()
}
...
function Test-R7CandidateV08Contract {
  param([object]$Candidate,[object]$SourceSet=$null)
  $errors=[Collections.Generic.List[string]]::new();$required=@('schema_id','schema_version','final_delivery_id','session_id','candidate_status','action_registry_version','presentation_registry_version','compiler_provenance','content_source_context','source_map','source_binding_digest','artifact_execution_contribution','external_activity_counts','execution_transparency','visual_route_summary','revision_context','delivery_payload')
  foreach($e in(Test-P0RequiredProperties $Candidate $required 'candidate_v08')){$errors.Add($e)};foreach($e in(Test-P0AllowedProperties $Candidate $required 'candidate_v08')){$errors.Add($e)};if($errors.Count){return [object[]]$errors.ToArray()}
  if($Candidate.schema_id-ne'taoge://schemas/final-delivery/typed-components/v0.8'-or$Candidate.schema_version-ne'typed_components_v0.8'){$errors.Add('candidate_v08_version_invalid')}
  if($Candidate.compiler_provenance.producer-ne'deterministic_compiler'-or$Candidate.compiler_provenance.compiler_version-ne'p0-deterministic-delivery-candidate-compiler-v0.8'){$errors.Add('candidate_v08_producer_invalid')}
  foreach($e in(Test-R7ContentSourceContextV07 $Candidate.content_source_context)){$errors.Add($e)}
  if($Candidate.execution_transparency.test_profile-notin@('production','no_provider','reuse_only')){$errors.Add('candidate_v08_test_profile_invalid')}
  if($Candidate.execution_transparency.image2_runtime_profile-ne'not_observable'){$errors.Add('candidate_v08_image2_profile_overclaimed')}
  foreach($route in @($Candidate.visual_route_summary)){
    if($route.source_class-notin@('source_bound_evidence','explicit_existing_asset','generated_context')){$errors.Add("candidate_v08_source_class_invalid:$($route.visual_task_id)")}
    if($route.source_class-eq'source_bound_evidence'-and($route.provider_or_capture-ne'source_capture'-or$route.evidence_parity_result-ne'match')){$errors.Add("candidate_v08_evidence_route_invalid:$($route.visual_task_id)")}
    if($route.source_class-eq'generated_context'-and$route.provider_or_capture-ne'image2'){$errors.Add("candidate_v08_generated_route_invalid:$($route.visual_task_id)")}
    if($route.source_class-eq'explicit_existing_asset'-and($route.provider_or_capture-ne'authorized_existing_asset'-or$null-eq$route.reuse_authorization_ref)){$errors.Add("candidate_v08_existing_route_invalid:$($route.visual_task_id)")}
  }
  if([int]$Candidate.revision_context.delivery_revision_no-lt1){$errors.Add('candidate_v08_revision_invalid')}
  if([int]$Candidate.revision_context.delivery_revision_no-gt1-and$null-eq$Candidate.revision_context.supersedes_delivery_ref){$errors.Add('candidate_v08_supersedes_delivery_missing')}
  foreach($e in(Test-P0RenderInputV05Contract $Candidate.delivery_payload)){$errors.Add("candidate_v08_delivery_payload:$e")}
  return [object[]]$errors.ToArray()
}
...
function Commit-R7DeterministicArtifact {
  ...
  if(Test-Path -LiteralPath $pointerPath){
    ...
    if([string]$plan.plan_schema_id-notin@('taoge://schemas/p0/session-execution-plan/v0.9','taoge://schemas/p0/session-execution-plan/v1.0','taoge://schemas/p0/session-execution-plan/v1.1','taoge://schemas/p0/session-execution-plan/v1.2','taoge://schemas/p0/session-execution-plan/v1.3','taoge://schemas/p0/session-execution-plan/v1.4')-or[int]$plan.plan_revision-le1-or[string]$existing.artifact_id-eq$ArtifactId){return New-R7RuntimeResult 'deterministic_current_conflict' 1 $existing @()}
    $pointerRevision=[int]$plan.plan_revision
  }elseif([string]$plan.plan_schema_id-in@('taoge://schemas/p0/session-execution-plan/v0.9','taoge://schemas/p0/session-execution-plan/v1.0','taoge://schemas/p0/session-execution-plan/v1.1','taoge://schemas/p0/session-execution-plan/v1.2','taoge://schemas/p0/session-execution-plan/v1.3','taoge://schemas/p0/session-execution-plan/v1.4')){$pointerRevision=[int]$plan.plan_revision}
  ...
  $pointer=[ordered]@{schema_id='taoge://schemas/r7/semantic-current-pointer/v0.1';schema_version='0.1';artifact_type=$ArtifactType;artifact_id=$ArtifactId;revision=$pointerRevision;revision_path=$revisionRelative;sha256=$digest;status=$Status;task_envelope_id="TASK-$($plan.session_id)-$NodeId-deterministic";submission_id="RUN-$($plan.session_id)-$NodeId-deterministic";producer_event_id=$predicted;committed_at=[DateTimeOffset]::UtcNow.ToString('o')}
...
function Invoke-R7CandidateCompile {
  ...
  try{$sourceSet=Get-R7CandidateSourceSet $sessionRoot;$candidate=New-R7CandidatePayload $ProjectRoot $sessionRoot $sourceSet;$errors=@($(if($candidate.schema_id-eq'taoge://schemas/final-delivery/typed-components/v0.9'){Test-R7CandidateV09Contract $candidate $sourceSet}elseif($candidate.schema_id-eq'taoge://schemas/final-delivery/typed-components/v0.8'){Test-R7CandidateV08Contract $candidate $sourceSet}elseif($candidate.schema_id-eq'taoge://schemas/final-delivery/typed-components/v0.7'){Test-R7CandidateV07Contract $candidate $sourceSet}else{Test-R7CandidateV06Contract $candidate}));if($errors.Count){return New-R7RuntimeResult 'candidate_integration_error' 1 $candidate $errors}}
  ...
  $nodeId=$(if($candidate.schema_id-eq'taoge://schemas/final-delivery/typed-components/v0.9'){'delivery_candidate_compile_h7'}else{'delivery_candidate_compile'})
  return Commit-R7DeterministicArtifact $ProjectRoot $sessionRoot $nodeId 'final_delivery_render_candidate' ([string]$candidate.final_delivery_id) $candidate ([string]$candidate.candidate_status) @($sourceSet.SourceMap|ForEach-Object{[string]$_.artifact_id}) $(if($candidate.schema_id-eq'taoge://schemas/final-delivery/typed-components/v0.9'){@('R3-C154','R7-C113','R7-C119','R7-C126')}elseif($candidate.schema_id-eq'taoge://schemas/final-delivery/typed-components/v0.8'){@('R6-C57','R3-C152','R7-C104','R7-C105')}elseif($candidate.schema_id-eq'taoge://schemas/final-delivery/typed-components/v0.7'){@('R7-F42','R7-F50','R7-F68','R7-F81')}else{@('R7-F09','R7-F10','R7-F11','R7-F13')})
}
...
function Get-R7VisualRouteHtml {
  param([object]$Candidate)
  $cards=[Collections.Generic.List[string]]::new()
  foreach($route in @($Candidate.visual_route_summary)){
    $authorization=if($null-eq$route.reuse_authorization_ref){'not_applicable'}else{[string]$route.reuse_authorization_ref.artifact_id}
    $cards.Add('<article class="card"><h3>'+[string](Encode-P0V2Html ([string]$route.visual_task_id))+'</h3><p>source class: '+[string](Encode-P0V2Html ([string]$route.source_class))+'</p><p>route: '+[string](Encode-P0V2Html ([string]$route.route_decision))+'</p><p>provider/capture: '+[string](Encode-P0V2Html ([string]$route.provider_or_capture))+'</p><p>evidence parity: '+[string](Encode-P0V2Html ([string]$route.evidence_parity_result))+'</p><p>reuse authorization: '+[string](Encode-P0V2Html $authorization)+'</p></article>')
  }
...
function Invoke-R7DeliveryRender {
  param([string]$ProjectRoot,[string]$Session)
  $sessionRoot=[IO.Path]::GetFullPath($Session)
  try{$current=Get-R7CandidateCurrentArtifact $sessionRoot 'final_delivery_render_candidate';$candidate=$current.Payload;$isV09=$candidate.schema_id-eq'taoge://schemas/final-delivery/typed-components/v0.9';$isV08=$candidate.schema_id-eq'taoge://schemas/final-delivery/typed-components/v0.8';$isV07=$candidate.schema_id-eq'taoge://schemas/final-delivery/typed-components/v0.7';$errors=@($(if($isV09){Test-R7CandidateV09Contract $candidate}elseif($isV08){Test-R7CandidateV08Contract $candidate}elseif($isV07){Test-R7CandidateV07Contract $candidate}else{Test-R7CandidateV06Contract $candidate}));if($errors.Count){return New-R7RuntimeResult 'candidate_integration_error' 1 $candidate $errors};$inner=$candidate.delivery_payload
    $views=New-P0V5ViewTexts $inner $sessionRoot $ProjectRoot;$fragmentRef=$(if($isV09){'templates/final-delivery/final-delivery.v0.9.business-fragment.html'}elseif($isV08){'compatibility/legacy-r7/templates/final-delivery/final-delivery.v0.8.fragment.html'}elseif($isV07){'compatibility/legacy-r7/templates/final-delivery/final-delivery.v0.7.hotspot-fragment.html'}else{'compatibility/legacy-r7/templates/final-delivery/final-delivery.v0.6.execution-fragment.html'});$fragmentPath=$(if($isV09){Join-Path $ProjectRoot $fragmentRef}else{Resolve-WorkflowCompatibilityAsset -ProjectRoot $ProjectRoot -AssetReference $fragmentRef -CallerRuntimeGeneration 'legacy_r7'});$fragment=Get-Content -Raw -Encoding UTF8 $fragmentPath;$fragment=$fragment.Replace('{{execution_contribution_cards}}',(Get-R7ExecutionContributionHtml $candidate.artifact_execution_contribution));if($isV07){$fragment=$fragment.Replace('{{hotspot_source_cards}}',(Get-R7HotspotSourceHtml $candidate $sessionRoot))};if($isV08){$sourceCards=if($candidate.content_source_context.content_origin-eq'hotspot_selected_topic'){Get-R7HotspotSourceHtml $candidate $sessionRoot}else{'<article class="card"><h3>Direct supplied draft</h3><p>Source is the current hash-bound user draft.</p></article>'};$fragment=$fragment.Replace('{{source_currentness_cards}}',$sourceCards).Replace('{{visual_route_cards}}',(Get-R7VisualRouteHtml $candidate))}
    if($isV09){if(-not(Get-Command ConvertTo-R7H7PlatformUnitsHtml -ErrorAction SilentlyContinue)){. (Join-Path $PSScriptRoot 'R7H7DeliveryContract.ps1')};$paths=$inner.delivery_revision.generated_view_paths;$htmlPathForLinks=Join-Path $sessionRoot ([string]$paths.final_html);$htmlBase=Split-Path -Parent $htmlPathForLinks;$sourceCards=if($candidate.content_source_context.content_origin-eq'hotspot_selected_topic'){Get-R7HotspotSourceHtml $candidate $sessionRoot}else{'<article class="card"><h3>用户直供稿</h3><p>来源为当前 hash 绑定的原稿。</p></article>'};$audit='<section id="content-structure"><h2>结构</h2>'+[string](ConvertTo-P0V5StructureHtml $inner)+'</section><section id="script-review"><h2>文案质检</h2>'+[string](ConvertTo-P0V5ReviewHtml $inner)+'</section><section id="visual-coverage"><h2>视觉覆盖</h2>'+[string](ConvertTo-P0V5CoverageSummaryHtml $inner)+[string](ConvertTo-P0V5BeatCardsHtml $inner)+'</section><section id="execution-transparency"><h2>执行贡献</h2>'+[string](Get-R7ExecutionContributionHtml $candidate.artifact_execution_contribution)+'</section><section id="source-currentness"><h2>来源</h2>'+$sourceCards+'</section><section id="visual-source-routing"><h2>视觉路由</h2>'+[string](Get-R7VisualRouteHtml $candidate)+'</section><section id="trace-links"><h2>追溯</h2>'+[string](ConvertTo-P0V3TraceHtml $inner $sessionRoot $htmlBase)+'</section>';$fragment=$fragment.Replace('{{delivery_title}}',[string](Encode-P0V2Html $candidate.delivery_title)).Replace('{{delivery_readiness}}',[string](Encode-P0V2Html $candidate.readiness_derivation.maximum_publish_readiness)).Replace('{{delivery_status_message}}','本页已按最终素材、平台物料和当前风险生成；发布仍由用户人工完成。').Replace('{{script_primary}}',('<h2>正式口播</h2><textarea class="copy-field script-field" readonly>'+[string](Encode-P0V2Html $inner.script_card.final_text)+'</textarea>')).Replace('{{visual_inserts_primary}}',('<h2>画中画成品</h2><div class="grid delivery-list">'+[string](ConvertTo-P0V4VisualInsertCardsHtml $inner $sessionRoot $htmlBase)+'</div>')).Replace('{{platform_delivery_primary}}',('<h2>平台物料</h2><div class="grid delivery-list">'+[string](ConvertTo-R7H7PlatformUnitsHtml $inner $sessionRoot $htmlBase)+'</div>')).Replace('{{warnings_primary}}',('<h2>发布前提醒</h2><div class="warning-list">'+[string](ConvertTo-P0V3WarningsHtml $inner)+'</div>')).Replace('{{actions_primary}}',('<h2>下一步</h2><div class="actions">'+[string](ConvertTo-P0V3ActionsHtml $inner)+'</div>')).Replace('{{audit_details}}',$audit)}
    $version=$(if($isV09){'0.9.0'}elseif($isV08){'0.8.0'}elseif($isV07){'0.7.0'}else{'0.6.0'});$html=$views.Html.Replace('data-template-version="0.5.0"',"data-template-version=`"$version`"");if($isV09){$mainStart=$html.IndexOf('<main ');$mainOpenEnd=$html.IndexOf('>',$mainStart)+1;$mainClose=$html.LastIndexOf('</main>');if($mainStart-lt0-or$mainOpenEnd-le0-or$mainClose-le$mainOpenEnd){throw 'h7_main_shell_missing'};$html=$html.Substring(0,$mainOpenEnd)+"`n"+$fragment+"`n"+$html.Substring($mainClose);$html=[regex]::Replace($html,'(?is)<title>.*?</title>','<title>'+[string](Encode-P0V2Html $candidate.delivery_title)+'</title>')}else{$html=$html.Replace('</main>',($fragment+"`n</main>"))};$paths=$inner.delivery_revision.generated_view_paths
    ...
    $htmlPath=Join-Path $sessionRoot ([string]$paths.final_html);$htmlErrors=@(Test-R7RenderedHtmlV06 (Get-Content -Raw -Encoding UTF8 $htmlPath) $htmlPath $sessionRoot $version);if(($isV07-or$isV08)-and((Get-Content -Raw -Encoding UTF8 $htmlPath)-notmatch'id="source-currentness"')){$htmlErrors+=@('current_business_section_missing:source-currentness')};if($isV08-and((Get-Content -Raw -Encoding UTF8 $htmlPath)-notmatch'id="visual-source-routing"')){$htmlErrors+=@('v08_business_section_missing:visual-source-routing')};if($htmlErrors.Count){throw ('rendered_html_invalid:'+($htmlErrors -join ';'))}
    ...
    $manifest=[ordered]@{schema_id="taoge://schemas/p0/delivery-revision/v$receiptVersion";schema_version=$receiptVersion;delivery_revision_id=[string]$inner.delivery_revision.delivery_revision_id;session_id=[string]$candidate.session_id;revision_no=$(if($isV08-or$isV09){[int]$candidate.revision_context.delivery_revision_no}else{1});revision_status='current';semantic_gate_status='pass';candidate_id=[string]$candidate.final_delivery_id;candidate_sha256=$candidateDigest;output_artifacts=[object[]]$outputs.ToArray();committed_at=[DateTimeOffset]::UtcNow.ToString('o')};Write-P0V2AtomicText (Join-Path $sessionRoot ([string]$paths.revision_manifest)) (ConvertTo-P0V2JsonText $manifest)
    $delivery=[ordered]@{schema_id="taoge://schemas/final-delivery/final-delivery/v$receiptVersion";schema_version=$receiptVersion;final_delivery_id=('DELIVERY-'+[string]$candidate.final_delivery_id);session_id=[string]$candidate.session_id;delivery_revision_no=$(if($isV08-or$isV09){[int]$candidate.revision_context.delivery_revision_no}else{1});supersedes_delivery_ref=$(if($isV08-or$isV09){$candidate.revision_context.supersedes_delivery_ref}else{$null});delivery_status=$(if($candidate.candidate_status -eq 'compiled_with_warnings'){'ready_with_warnings'}else{'delivery_ready'});candidate_ref=[ordered]@{artifact_id=[string]$candidate.final_delivery_id;sha256=$candidateDigest};renderer_version="final-delivery-renderer-v$receiptVersion";template_version="final-delivery-template-v$receiptVersion";html_path=[string]$paths.final_html;html_sha256=$htmlDigest;render_receipt_path='deliverables/p0/render-receipt.json';revision_manifest_path=[string]$paths.revision_manifest}
  }catch{return New-R7RuntimeResult 'render_compile_error' 1 $null @($_.Exception.Message)}
  $nodeId=$(if($isV09){'final_delivery_render_h7'}else{'final_delivery_render'});return Commit-R7DeterministicArtifact $ProjectRoot $sessionRoot $nodeId 'final_delivery' ([string]$delivery.final_delivery_id) ([pscustomobject](($delivery|ConvertTo-Json -Depth 20)|ConvertFrom-Json)) ([string]$delivery.delivery_status) @([string]$candidate.final_delivery_id) $(if($isV09){@('R7-C113','R7-C114','R7-C119','R7-C124')}else{@('R7-H4-RENDER','R7-H4-LINKS','R7-H4-RECEIPT')})
}
...
function Invoke-R7DeterministicNode {
  ...
  switch([string]$step.node_id){'visual_asset_finalize'{if(-not(Get-Command Invoke-R7H7AssetFinalize -ErrorAction SilentlyContinue)){. (Join-Path $PSScriptRoot 'R7H7DeliveryContract.ps1')};return Invoke-R7H7AssetFinalize $ProjectRoot $sessionRoot ([string]$step.node_id)}'visual_asset_finalize_l3'{if(-not(Get-Command Invoke-R7H7AssetFinalize -ErrorAction SilentlyContinue)){. (Join-Path $PSScriptRoot 'R7H7DeliveryContract.ps1')};return Invoke-R7H7AssetFinalize $ProjectRoot $sessionRoot ([string]$step.node_id)}'delivery_candidate_compile'{return Invoke-R7CandidateCompile $ProjectRoot $sessionRoot}'delivery_candidate_compile_h7'{return Invoke-R7CandidateCompile $ProjectRoot $sessionRoot}'final_delivery_render'{return Invoke-R7DeliveryRender $ProjectRoot $sessionRoot}'final_delivery_render_h7'{return Invoke-R7DeliveryRender $ProjectRoot $sessionRoot}'viewport_acceptance'{if(-not(Get-Command Invoke-R7ViewportAcceptance -ErrorAction SilentlyContinue)){. (Join-Path $PSScriptRoot 'R7ViewportRuntime.ps1')};return Invoke-R7ViewportAcceptance $ProjectRoot $sessionRoot}'viewport_acceptance_h7'{if(-not(Get-Command Invoke-R7ViewportAcceptance -ErrorAction SilentlyContinue)){. (Join-Path $PSScriptRoot 'R7ViewportRuntime.ps1')};return Invoke-R7ViewportAcceptance $ProjectRoot $sessionRoot}'hotspot_research_request_commit'{if(-not(Get-Command Invoke-R7HotspotDeterministicNode -ErrorAction SilentlyContinue)){. (Join-Path $PSScriptRoot 'R7HotspotRuntime.ps1')};return Invoke-R7HotspotDeterministicNode $ProjectRoot $sessionRoot ([string]$step.node_id)}'topic_panel_projection'{if(-not(Get-Command Invoke-R7HotspotDeterministicNode -ErrorAction SilentlyContinue)){. (Join-Path $PSScriptRoot 'R7HotspotRuntime.ps1')};return Invoke-R7HotspotDeterministicNode $ProjectRoot $sessionRoot ([string]$step.node_id)}'selected_topic_source_commit'{if(-not(Get-Command Invoke-R7HotspotDeterministicNode -ErrorAction SilentlyContinue)){. (Join-Path $PSScriptRoot 'R7HotspotRuntime.ps1')};return Invoke-R7HotspotDeterministicNode $ProjectRoot $sessionRoot ([string]$step.node_id)}'delivery_topic_freshness_apply'{if(-not(Get-Command Invoke-R7HotspotDeterministicNode -ErrorAction SilentlyContinue)){. (Join-Path $PSScriptRoot 'R7HotspotRuntime.ps1')};return Invoke-R7HotspotDeterministicNode $ProjectRoot $sessionRoot ([string]$step.node_id)}'final_delivery_decision_apply'{if(-not(Get-Command Invoke-R8FinalDeliveryDecisionApply -ErrorAction SilentlyContinue)){. (Join-Path $PSScriptRoot 'R8HumanGateRuntime.ps1')};return Invoke-R8FinalDeliveryDecisionApply $ProjectRoot $sessionRoot}default{return New-R7RuntimeResult 'deterministic_node_not_compiled' 2 $step @([string]$step.node_id)}}
}
