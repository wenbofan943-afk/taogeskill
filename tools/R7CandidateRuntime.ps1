...
function Get-R7CandidateSourceSet {
  ...
  $plan=Read-R7JsonFile (Join-Path $SessionRoot 'intermediate/p0/session-execution-plan.json');$isHotspot=[string]$plan.blueprint_id-like'hotspot_to_delivery_single_*';$isH7=[string]$plan.plan_schema_id-eq'taoge://schemas/p0/session-execution-plan/v1.0'
...
function New-R7CandidatePayload {
  ...
  if($isV09){
    ...
    if([string]$deliveryAssetSet.finalize_status-notin@('finalized','finalized_with_warnings')){throw 'candidate_delivery_asset_set_not_finalized'}
  }
  ...
  $isAssetSetV03=[string]$assetSet.schema_id-eq'taoge://schemas/r7/image-asset-set/v0.3';$isAssetSetV02=[string]$assetSet.schema_id-in@('taoge://schemas/r7/image-asset-set/v0.2','taoge://schemas/r7/image-asset-set/v0.3')
  $assetById=@{};foreach($candidateAsset in @($assetSet.assets)){$assetById[[string]$candidateAsset.asset_id]=$candidateAsset}
  $assetsForDelivery=if($isAssetSetV03){@($deliveryAssetSet.delivery_assets|ForEach-Object{$assetById[[string]$_.delivery_asset_ref.asset_id]})}else{@($assetSet.assets)}
  ...
  $assetSetClaimsReady=[string]$assetSet.asset_set_status -in @('materialized','ready_with_warnings','materialized_waiting_finalize','ready_with_warnings_waiting_finalize')
...
function Commit-R7DeterministicArtifact {
  ...
  if(Test-Path -LiteralPath $pointerPath){
    ...
    if([string]$plan.plan_schema_id-notin@('taoge://schemas/p0/session-execution-plan/v0.9','taoge://schemas/p0/session-execution-plan/v1.0')-or[int]$plan.plan_revision-le1-or[string]$existing.artifact_id-eq$ArtifactId){return New-R7RuntimeResult 'deterministic_current_conflict' 1 $existing @()}
    $pointerRevision=[int]$plan.plan_revision
  }elseif([string]$plan.plan_schema_id-in@('taoge://schemas/p0/session-execution-plan/v0.9','taoge://schemas/p0/session-execution-plan/v1.0')){$pointerRevision=[int]$plan.plan_revision}
