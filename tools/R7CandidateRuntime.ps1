...
function New-R7CandidatePayload {
  ...
  $taskSummaries=[Collections.Generic.List[object]]::new();foreach($task in @($ledger.accepted_visual_tasks)){$taskSummaries.Add([ordered]@{visual_task_id=[string]$task.visual_task_id;disposition=[string]$task.disposition;capture_mode=[string]$task.capture_mode;asset_status=[string]$task.task_status;provider_attempt_count=$(if([string]$task.disposition -eq 'generate_visual'){[int]$assetSet.provider_invocation_count}else{0});source_capture_attempt_count=0})}
...
function Invoke-R7DeterministicNode {
  ...
  switch([string]$step.node_id){'delivery_candidate_compile'{return Invoke-R7CandidateCompile $ProjectRoot $sessionRoot}'final_delivery_render'{return Invoke-R7DeliveryRender $ProjectRoot $sessionRoot}default{return New-R7RuntimeResult 'deterministic_node_not_compiled' 2 $step @([string]$step.node_id)}}
}
