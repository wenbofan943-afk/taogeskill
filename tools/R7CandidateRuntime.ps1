...
function Get-R7CandidateSourceSet {
  ...
  $plan=Read-R7JsonFile (Join-Path $SessionRoot 'intermediate/p0/session-execution-plan.json');$isHotspot=[string]$plan.blueprint_id-like'hotspot_to_delivery_single_*'
...
function New-R7CandidatePayload {
  ...
  $s=$SourceSet.Sources;$plan=Read-R7JsonFile (Join-Path $SessionRoot 'intermediate/p0/session-execution-plan.json');$isV08=[string]$plan.plan_schema_id-eq'taoge://schemas/p0/session-execution-plan/v0.9'
...
function Commit-R7DeterministicArtifact {
  ...
  if(Test-Path -LiteralPath $pointerPath){
    ...
    if([string]$plan.plan_schema_id-ne'taoge://schemas/p0/session-execution-plan/v0.9'-or[int]$plan.plan_revision-le1-or[string]$existing.artifact_id-eq$ArtifactId){return New-R7RuntimeResult 'deterministic_current_conflict' 1 $existing @()}
    $pointerRevision=[int]$plan.plan_revision
  }elseif([string]$plan.plan_schema_id-eq'taoge://schemas/p0/session-execution-plan/v0.9'){$pointerRevision=[int]$plan.plan_revision}
...
function Invoke-R7DeliveryRender {
  ...
  try{$current=Get-R7CandidateCurrentArtifact $sessionRoot 'final_delivery_render_candidate';$candidate=$current.Payload;$isV08=$candidate.schema_id-eq'taoge://schemas/final-delivery/typed-components/v0.8';$isV07=$candidate.schema_id-eq'taoge://schemas/final-delivery/typed-components/v0.7';$errors=@($(if($isV08){Test-R7CandidateV08Contract $candidate}elseif($isV07){Test-R7CandidateV07Contract $candidate}else{Test-R7CandidateV06Contract $candidate}));if($errors.Count){return New-R7RuntimeResult 'candidate_integration_error' 1 $candidate $errors};$inner=$candidate.delivery_payload
    ...
    $manifest=[ordered]@{schema_id="taoge://schemas/p0/delivery-revision/v$receiptVersion";schema_version=$receiptVersion;delivery_revision_id=[string]$inner.delivery_revision.delivery_revision_id;session_id=[string]$candidate.session_id;revision_no=$(if($isV08){[int]$candidate.revision_context.delivery_revision_no}else{1});revision_status='current';semantic_gate_status='pass';candidate_id=[string]$candidate.final_delivery_id;candidate_sha256=$candidateDigest;output_artifacts=[object[]]$outputs.ToArray();committed_at=[DateTimeOffset]::UtcNow.ToString('o')};Write-P0V2AtomicText (Join-Path $sessionRoot ([string]$paths.revision_manifest)) (ConvertTo-P0V2JsonText $manifest)
    $delivery=[ordered]@{schema_id="taoge://schemas/final-delivery/final-delivery/v$receiptVersion";schema_version=$receiptVersion;final_delivery_id=('DELIVERY-'+[string]$candidate.final_delivery_id);session_id=[string]$candidate.session_id;delivery_revision_no=$(if($isV08){[int]$candidate.revision_context.delivery_revision_no}else{1});supersedes_delivery_ref=$(if($isV08){$candidate.revision_context.supersedes_delivery_ref}else{$null});delivery_status=$(if($candidate.candidate_status -eq 'compiled_with_warnings'){'ready_with_warnings'}else{'delivery_ready'});candidate_ref=[ordered]@{artifact_id=[string]$candidate.final_delivery_id;sha256=$candidateDigest};renderer_version="final-delivery-renderer-v$receiptVersion";template_version="final-delivery-template-v$receiptVersion";html_path=[string]$paths.final_html;html_sha256=$htmlDigest;render_receipt_path='deliverables/p0/render-receipt.json';revision_manifest_path=[string]$paths.revision_manifest}
