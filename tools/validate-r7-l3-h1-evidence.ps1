...
try{
  ...
  if($cliExit-ne0){throw "baseline_cli_failed:${cliExit}:$([string]::Join(';',@($cliOutput)))"};$baseline=Read-R7MaturityJson $baselinePath;if(@($baseline.contract_surfaces).Count-ne4){throw 'baseline_contract_surface_count_invalid'};$registryDocument=Read-R7MaturityJson $registryPath;Add-CaseResult 'baseline_capabilities' ([string]@($registryDocument.capabilities).Count) ([string]@($baseline.capabilities).Count) 'capability_contract'
