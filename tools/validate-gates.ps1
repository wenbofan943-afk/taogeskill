  ...
  foreach ($gate in $targetGates) {
    switch ($gate) {
      ...
      'product_contract_compilation_gate' {
        ...
        Add-GateCheck $checks 'PRODUCT-CONTRACT-012' $(if($r7H3Succeeded-and$r7H3Text.Contains('R7_H3_PRODUCER_CHECK_RESULT=pass')-and$r7H3Text.Contains('R7_H3_ADAPTER_COUNT=15')-and$r7H3Text.Contains('R7_H3_FIXTURE_COUNT=3')){'pass'}else{'fail'}) $r7H3Text 'Compile R7-H3 producer adapters, deterministic submission building, native status mapping, keep-current and waiting cursor semantics.'
