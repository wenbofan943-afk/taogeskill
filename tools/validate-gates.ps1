  ...
  foreach ($gate in $targetGates) {
    switch ($gate) {
      ...
      'product_contract_compilation_gate' {
        ...
        Add-GateCheck $checks 'PRODUCT-CONTRACT-012' $(if($r7H3Succeeded-and$r7H3Text.Contains('R7_H3_PRODUCER_CHECK_RESULT=pass')-and$r7H3Text.Contains("R7_H3_ADAPTER_COUNT=$r7ExpectedAdapterCount")-and$r7H3Text.Contains('R7_H3_FIXTURE_COUNT=3')){'pass'}else{'fail'}) "$r7H3Text;registry_derived_adapter_count=$r7ExpectedAdapterCount" 'Compile all producer adapters declared by the current registry, deterministic submission building, native status mapping, keep-current and waiting cursor semantics.'
        ...
        Add-GateCheck $checks 'PRODUCT-CONTRACT-013' $(if($r7H4Succeeded-and$r7H4Text.Contains('R7_H4_CANDIDATE_CHECK_RESULT=pass')-and$r7H4Text.Contains('R7_H4_FIXTURE_COUNT=9')){'pass'}else{'fail'}) $r7H4Text 'Compile R7-H4 candidate v0.6, multi-beat occurrence ownership, per-rendition review binding, renderer v0.6 and deterministic event/source-map closure.'
        ...
        Add-GateCheck $checks 'PRODUCT-CONTRACT-016' $(if($r7H6ASucceeded-and$r7H6AText.Contains('R7_H6A_HOTSPOT_RESULT=pass')-and$r7H6AText.Contains('R7_H6A_FIXTURE_COUNT=31')){'pass'}else{'fail'}) $r7H6AText 'Compile the R7-H6A request-to-draft hotspot front chain and keep H6B/H6C boundaries explicit.'
      }
