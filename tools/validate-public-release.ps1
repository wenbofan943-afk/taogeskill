...
'@
  ...
  if(Test-Path -LiteralPath $r7H4Path){& $r7H4Path -WorkRoot (Join-Path $checkerReportRoot 'r7-h4-candidate-work') -HumanReportPath (Join-Path $checkerReportRoot 'r7-h4-candidate-report.md') -MachineReportPath (Join-Path $checkerReportRoot 'r7-h4-candidate-report.json')|Out-Null;if(-not $?){$r7H4Status='fail';$r7H4Evidence=@('checker-reports\r7-h4-candidate-report.json')}}else{$r7H4Status='fail';$r7H4Evidence=@('tools\validate-r7-h4-candidate-runtime.ps1','tools\R7CandidateRuntime.ps1','routes\r7-delivery-presentation-registry.yaml','templates\schema\final-delivery\typed-components.v0.6.schema.json')}
  ...
  if(Test-Path -LiteralPath $r7H7Path){$LASTEXITCODE=0;& $r7H7Path -ProjectRoot $target|Out-Null;if($LASTEXITCODE -ne 0){$r7H7Status='fail';$r7H7Evidence=@('state\checks\r7-h7-delivery-contract-report.json')}}else{$r7H7Status='fail';$r7H7Evidence=@('tools\validate-r7-h7-delivery-contract.ps1','tools\R7H7DeliveryContract.ps1','templates\schema\r7\business-delivery-acceptance.v0.1.schema.json')}
  ...
  if(Test-Path -LiteralPath $r7CliExitPath){$LASTEXITCODE=0;& $r7CliExitPath -ReportRoot 'state/checks/public-r7-cli-exit-contract'|Out-Null;if($LASTEXITCODE -ne 0){$r7CliExitStatus='fail';$r7CliExitEvidence=@('state\checks\public-r7-cli-exit-contract')}}else{$r7CliExitStatus='fail';$r7CliExitEvidence=@('tools\validate-r7-cli-exit-contract.ps1')}
