...
try{
  ...
  $actual25=if($built.ResultCode-eq'submission_built'-and[int](Read-R7JsonFile (Join-Path $semantic.Session $built.Data.SubmissionPath)).output_revision-eq1-and[string]$prepared.Data.Task.action_registry_version-eq'r7-action-registry-v0.1'){'submission_revision_1'}elseif([string]$prepared.Data.Task.action_registry_version-ne'r7-action-registry-v0.1'){'direct_action_registry_not_pinned'}else{[string]$built.ResultCode};$results.Add((New-R7H5AResult 'R7-F25-SEMANTIC-BEAT' submission_revision_1 $actual25 @($built.Errors)))
