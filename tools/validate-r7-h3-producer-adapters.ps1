...
try{
  ...
  $visualSelector=@($registries.Selectors.selectors|Where-Object{$_.selector_id-eq'current_account_visual_identity'})|Select-Object -First 1
  $visualSelectorErrors=@();if($null-eq$visualSelector){$visualSelectorErrors+='visual_identity_selector_missing'}elseif(@($visualSelector.path_candidates).Count-lt2-or[string]$visualSelector.path_candidates[0]-ne'intermediate/account-startup/visual-delivery-account-snapshot.v0.2.json'-or[string]$visualSelector.path_candidates[1]-ne'intermediate/account-startup/account-snapshot.v0.2.json'){$visualSelectorErrors+='visual_snapshot_precedence_invalid'}
  $results.Add((New-R7H3Result 'R7-H3-VISUAL-SNAPSHOT-PRECEDENCE' pass $(if($visualSelectorErrors.Count){'fail'}else{'pass'}) $visualSelectorErrors))
