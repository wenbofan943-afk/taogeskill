...
try {
  ...
  $requirements = @(
    @{ id = 'ALPHA-README-001'; path = 'README.md'; needles = @('0.1.0-alpha.1', 'Alpha', '不是已发布的 GitHub Release', '不是生产级自动化 runner', '不能自动发布内容') },
    @{ id = 'ALPHA-INSTALL-001'; path = 'INSTALL.md'; needles = @('Alpha', '不是 GitHub Release', '不包含生产 runner', '先跑 `examples/`', 'validate-regression-suite.ps1') },
    @{ id = 'ALPHA-RELEASE-001'; path = 'RELEASE_NOTES.md'; needles = @('local alpha candidate', 'not a published GitHub release', 'not treat it as a production workflow engine', 'Validation-only GitHub Actions workflow exists') },
