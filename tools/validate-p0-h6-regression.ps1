...
try{
  ...
  Add-H6Check $assertions $errors 'content_driven_cardinality' ([int]$visual.derived_visual_count-eq$accepted.Count-and$accepted.Count-eq$prompts.Count-and$accepted.Count-eq$assets.Count-and[int]$selection.accepted_task_count-eq$accepted.Count-and[int]$selection.selected_asset_count-eq$assets.Count) "accepted=$($accepted.Count);rejected=$($rejected.Count);derived=$($visual.derived_visual_count)"
  ...
  $manifestCountPattern='image_provider_invocation_count:\s*'+[regex]::Escape([string]$accepted.Count)+'\s*'
  Add-H6Check $assertions $errors 'manifest_h6_boundary' ($manifest-match'task_context_type: p0_h6_real_image_regression'-and$manifest-match'run_mode: phase_2_real_regression_with_new_image2_assets'-and$manifest-match'image_provider_invoked: true'-and$manifest-match$manifestCountPattern-and$manifest-match'publishing_invoked: false') $manifestPath
  ...
  foreach($asset in $assets){
    ...
    $recordText=Get-Content -LiteralPath $record -Raw -Encoding UTF8;if($recordText-notmatch'provider_outcome_status: succeeded'-or$recordText-notmatch'postprocess_status: completed'-or$recordText-notmatch'interruption_recovery_policy: reconcile_existing_output_before_retry'){$assetFailures.Add("recovery_contract:$($asset.asset_id)")}
  ...
  Add-H6Check $assertions $errors 'derived_covers_complete' ($covers.Count-eq[int]$selection.derived_cover_count-and$coverFailures.Count-eq0) "covers=$($covers.Count)"
  ...
  Add-H6Check $assertions $errors 'candidate_render_input_digest_match' ((Get-H6CheckHash (Join-Path $session 'deliverables/p0/final-delivery-render-candidate.json'))-eq(Get-H6CheckHash (Join-Path $session 'deliverables/p0/final-delivery-render-input.json'))) 'candidate and compiled input are identical'
  ...
  Add-H6Check $assertions $errors 'render_cards_match_assets' (@($renderInput.pip_cards).Count-eq$assets.Count-and@($renderInput.cover_cards).Count-eq$covers.Count-and@($renderInput.pip_cards|Where-Object{$_.asset_status-ne'generated'}).Count-eq0-and@($renderInput.cover_cards|Where-Object{$_.asset_status-ne'generated'}).Count-eq0) "pip=$(@($renderInput.pip_cards).Count);cover=$(@($renderInput.cover_cards).Count)"
  ...
  Add-H6Check $assertions $errors 'receipt_contains_all_generated_assets' (@($receipt.included_asset_ids).Count-eq($assets.Count+$covers.Count)-and@($assets|Where-Object{@($receipt.included_asset_ids)-notcontains$_.asset_id}).Count-eq0-and@($covers|Where-Object{@($receipt.included_asset_ids)-notcontains$_.asset_id}).Count-eq0) "assets=$(@($receipt.included_asset_ids).Count);expected=$($assets.Count+$covers.Count)"
