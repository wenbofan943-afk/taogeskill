...
function Test-P0PlanContract {
  ...
  $allowed = $required
  foreach ($error in (Test-P0RequiredProperties $Plan $required 'plan')) { $errors.Add($error) }
  foreach ($error in (Test-P0AllowedProperties $Plan $allowed 'plan')) { $errors.Add($error) }
  ...
  foreach ($step in @($Plan.steps)) {
    foreach ($error in (Test-P0RequiredProperties $step @('step_id','step_kind','failure_route','retry_policy') 'step')) { $errors.Add($error) }
    foreach ($error in (Test-P0AllowedProperties $step $stepAllowed 'step')) { $errors.Add($error) }
    ...
    foreach ($error in (Test-P0RequiredProperties $policy @('mode','automatic_retries','max_attempts','idempotency_scope') 'retry_policy')) { $errors.Add($error) }
    foreach ($error in (Test-P0AllowedProperties $policy @('mode','automatic_retries','max_attempts','idempotency_scope') 'retry_policy')) { $errors.Add($error) }
...
function Test-P0EventLogContract {
  ...
  foreach ($event in @($Events)) {
    $index++
    foreach ($error in (Test-P0RequiredProperties $event $required 'event')) { $errors.Add($error) }
    foreach ($error in (Test-P0AllowedProperties $event $allowed 'event')) { $errors.Add($error) }
    ...
    if (Test-P0HasProperty $event 'failure') {
      ...
      foreach ($error in (Test-P0RequiredProperties $failure @('failure_category','retryability','attempt_no','max_attempts','recovery_action') 'failure')) { $errors.Add($error) }
      foreach ($error in (Test-P0AllowedProperties $failure @('failure_category','retryability','attempt_no','max_attempts','next_retry_not_before','recovery_action') 'failure')) { $errors.Add($error) }
...
function Test-P0LineageContract {
  ...
  $errors = [System.Collections.Generic.List[string]]::new()
  foreach ($error in (Test-P0RequiredProperties $Document @('schema_id','schema_version','artifact_lineage_manifest') 'lineage_document')) { $errors.Add($error) }
  foreach ($error in (Test-P0AllowedProperties $Document @('schema_id','schema_version','artifact_lineage_manifest') 'lineage_document')) { $errors.Add($error) }
  if ($errors.Count) { return [object[]]$errors.ToArray() }
  ...
  foreach ($error in (Test-P0RequiredProperties $lineage $required 'lineage')) { $errors.Add($error) }
  foreach ($error in (Test-P0AllowedProperties $lineage $allowed 'lineage')) { $errors.Add($error) }
...
function Test-P0ArtifactCheckSetContract {
  param([object]$Document)
  $errors = [System.Collections.Generic.List[string]]::new()
  foreach ($error in (Test-P0RequiredProperties $Document @('schema_id','schema_version','check_set_id','checks') 'check_set')) { $errors.Add($error) }
  foreach ($error in (Test-P0AllowedProperties $Document @('schema_id','schema_version','check_set_id','checks') 'check_set')) { $errors.Add($error) }
  if ($errors.Count) { return [object[]]$errors.ToArray() }
  ...
  foreach ($check in @($Document.checks)) {
    ...
    foreach ($error in (Test-P0RequiredProperties $check $required 'artifact_check')) { $errors.Add($error) }
    foreach ($error in (Test-P0AllowedProperties $check $required 'artifact_check')) { $errors.Add($error) }
...
function Test-P0RenderInputContract {
  ...
  foreach ($error in (Test-P0RequiredProperties $Document $required 'render_input')) { $errors.Add($error) }
  foreach ($error in (Test-P0AllowedProperties $Document ($required + @('cover_empty_reason','pip_empty_reason')) 'render_input')) { $errors.Add($error) }
  ...
  foreach ($error in (Test-P0RequiredProperties $Document.topic @('title','why_now','content_format') 'topic')) { $errors.Add($error) }
  foreach ($error in (Test-P0AllowedProperties $Document.topic @('title','why_now','content_format') 'topic')) { $errors.Add($error) }
  foreach ($error in (Test-P0RequiredProperties $Document.script_card @('card_id','card_type','status','source_artifact_ids','final_text','copy_label','source_draft_id') 'script_card')) { $errors.Add($error) }
  foreach ($error in (Test-P0AllowedProperties $Document.script_card @('card_id','card_type','status','source_artifact_ids','final_text','copy_label','source_draft_id','estimated_duration_seconds','revision_note') 'script_card')) { $errors.Add($error) }
  ...
  foreach ($error in (Test-P0RequiredProperties $Document.production_status $productionFields 'production_status')) { $errors.Add($error) }
  foreach ($error in (Test-P0AllowedProperties $Document.production_status $productionFields 'production_status')) { $errors.Add($error) }
  ...
  foreach ($card in @($Document.cover_cards)) {
    foreach ($error in (Test-P0CardBase $card 'cover')) { $errors.Add($error) }
    foreach ($error in (Test-P0AllowedProperties $card $coverAllowed 'cover_card')) { $errors.Add($error) }
    ...
    foreach ($error in (Test-P0AssetCard $card 'cover')) { $errors.Add($error) }
  ...
  foreach ($card in @($Document.pip_cards)) {
    foreach ($error in (Test-P0CardBase $card 'picture_in_picture')) { $errors.Add($error) }
    foreach ($error in (Test-P0AllowedProperties $card $pipAllowed 'pip_card')) { $errors.Add($error) }
    ...
    foreach ($error in (Test-P0AssetCard $card 'picture_in_picture')) { $errors.Add($error) }
    if ($cardIds.ContainsKey([string]$card.card_id)) { $errors.Add("card_id_duplicate:$($card.card_id)") } else { $cardIds[[string]$card.card_id] = $true }
  }
  ...
  foreach ($card in @($Document.platform_cards)) {
    foreach ($error in (Test-P0CardBase $card 'platform')) { $errors.Add($error) }
    foreach ($error in (Test-P0RequiredProperties $card @('platform','cover_title','video_title','publish_description','hashtags','publish_readiness') 'platform_card')) { $errors.Add($error) }
    foreach ($error in (Test-P0AllowedProperties $card $platformAllowed 'platform_card')) { $errors.Add($error) }
  ...
  foreach ($card in @($Document.trace_cards)) {
    foreach ($error in (Test-P0CardBase $card 'trace')) { $errors.Add($error) }
    foreach ($error in (Test-P0RequiredProperties $card @('artifact_type','artifact_id','label','relative_path','materialization_status') 'trace_card')) { $errors.Add($error) }
    foreach ($error in (Test-P0AllowedProperties $card $traceAllowed 'trace_card')) { $errors.Add($error) }
  ...
  foreach ($card in @($Document.action_cards)) {
    foreach ($error in (Test-P0CardBase $card 'action')) { $errors.Add($error) }
    foreach ($error in (Test-P0RequiredProperties $card @('action','label','instruction','reply_example') 'action_card')) { $errors.Add($error) }
    foreach ($error in (Test-P0AllowedProperties $card $actionAllowed 'action_card')) { $errors.Add($error) }
...
function Test-P0CompatibilityMatrixContract {
  param([object]$Document)
  $errors = [System.Collections.Generic.List[string]]::new()
  foreach ($error in (Test-P0RequiredProperties $Document @('matrix_id','matrix_version','current_workflow_definition_version','entries') 'compatibility_matrix')) { $errors.Add($error) }
  foreach ($error in (Test-P0AllowedProperties $Document @('matrix_id','matrix_version','current_workflow_definition_version','entries') 'compatibility_matrix')) { $errors.Add($error) }
  if ($errors.Count) { return [object[]]$errors.ToArray() }
  ...
  foreach ($entry in @($Document.entries)) {
    ...
    foreach ($error in (Test-P0RequiredProperties $entry $fields 'compatibility_entry')) { $errors.Add($error) }
    foreach ($error in (Test-P0AllowedProperties $entry $fields 'compatibility_entry')) { $errors.Add($error) }
