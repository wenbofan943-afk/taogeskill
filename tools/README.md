...
| `WorkflowKernelSessionEntry.ps1` | runtime / architecture | M4 current IR 代际策略、session root、既有 binding 或 legacy plan | immutable runtime binding / read-only resume decision | session `intermediate/workflow-kernel/` |
...
`compile-workflow-ir.ps1` 从 `current-workflow-ir.json`、`component-catalog.json`、`compatibility-catalog.json` 生成 blueprint / stage / component / compatibility 视图，并逐节点反查 legacy R7 v0.6 的输入 selector、结果状态、输出合同、重试和实现入口。M4 后它同时校验新 session 代际策略、回滚范围和禁止原地迁移；parity report 最后写入，负例不得留下可误判为成功的派生视图。它不创建 session、不调用 worker/provider。`validate-workflow-ir-m1.ps1` 在 Windows PowerShell 5.1 中执行 1 个正例和 7 个 mutation 负例。
...
`WorkflowKernelSessionEntry.ps1` / `invoke-workflow-session-entry.ps1` 是 M4 唯一代际入口。新 session 在业务执行前先写 immutable binding，再写 SHA256 commit marker；重复 start byte-stable，resume 只服从已提交 binding。没有 binding 但存在 version-pinned R7 plan 的旧 session 只读路由到 `legacy_r7`。回滚只改变未来新 session，不修改已存在的 kernel session。`validate-workflow-kernel-m4.ps1` 在 Windows PowerShell 5.1 下执行 19 个正反 fixture，并验证 current binding 会被 legacy R7 coordinator 拒绝。M4 未执行语义 worker、网络、provider 或私有账号，也不构成 runtime certification。
