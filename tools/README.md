...
| `invoke-r8-h5r5-record-human-verdict.ps1` | dev / test | 只消费 ready blind pair 和人类 A/B/tie 请求，复核 presentation digest 后写不可变 human verdict；不读取 allocation mapping | console result | `state/checks/r8/{evaluation_id}/{attempt_id}/cases/{case_id}/human-verdict.json` |
| `invoke-r8-h5r5-finalize.ps1` | dev / test | 人类 verdict 提交后才读取私有 allocation，汇总 9 案 machine/comparability/human 证据，按 C60/C67 写唯一 finalization 和不重算结论的 state projection | console result | `state/checks/r8/{evaluation_id}/{attempt_id}/{evaluation-finalization.json,h5-state-projection.json}` |
| `validate-r8-h5r5-finalization-runtime.ps1` | dev / test | H5R5 waiting_human、不可变 verdict、mapping commitment、三类 false-success、router 0 样本 insufficient 与 finalization-only projection 的 PS5.1 门禁 | console report | `state/checks/r8-h5r5-finalization-runtime-report.json` |
...
| `compile-workflow-ir.ps1` | dev / architecture | 三份机器真源；legacy parity 输入只经 M5 compatibility loader 读取 | 4 份 current 派生视图 | `state/checks/workflow-kernel-m1/current/workflow-ir-parity-report.json` |
...
| `WorkflowKernelRuntime.ps1` | dev / architecture | current Workflow IR + component catalog + validated direct result envelopes | isolated artifact/event/state/resume/parity evidence | `state/checks/**/shadow/` |
| `WorkflowKernelHotspotRuntime.ps1` | dev / architecture | M3 hotspot start/resume command + typed results + external activity records | isolated append command/event/artifact/projection evidence | `state/checks/**/shadow/` |
| `invoke-workflow-kernel-shadow.ps1` | dev / architecture | M2 direct request、M3 hotspot command 或既有 shadow evidence | route run / projection rebuild result | isolated shadow root |
| `validate-workflow-kernel-m2.ps1` | dev / architecture | M2 direct baseline + 13 failure mutations + Windows PowerShell 5.1 | console report | `state/checks/workflow-kernel-m2-fixture-report.json` |
| `validate-workflow-kernel-m3.ps1` | dev / architecture | M3 hotspot frozen baseline + wait/resume/replan + false-success mutations + Windows PowerShell 5.1 | machine report | `state/checks/workflow-kernel-m3-fixture-report.json` |
| `WorkflowKernelSessionEntry.ps1` | runtime / architecture | M5 current IR 代际策略、session root、既有 binding 或 legacy plan | v0.2 immutable runtime binding / read-only resume decision | session `intermediate/workflow-kernel/` |
| `invoke-workflow-session-entry.ps1` | runtime / architecture | start / resume、session identity、route 与调用方时间 | generation-bound entry decision | session binding 或只读 legacy decision |
...
| `WorkflowCompatibilityLoader.ps1` | runtime / architecture | M5 compatibility catalog、version-pinned legacy plan 或 cataloged legacy asset | read-only compatibility source / resolution；current caller fail-closed | none |
| `validate-workflow-kernel-m5.ps1` | dev / architecture | M5 16 个 current 隔离、legacy 只读恢复、未知/错路由/未登记资产阻断 fixture + Windows PowerShell 5.1 | machine report | `state/checks/workflow-kernel-m5-fixture-report.json` |
...
`WorkflowKernelRuntime.ps1` / `invoke-workflow-kernel-shadow.ps1` 只实现 M2 direct 正向控制面 shadow：接收已通过组件 validator 的 typed result envelope，在 `state/checks/` 隔离写入 immutable artifact、append-only event、state / resume projection 和 parity report，并停在 `final_decision / waiting_human`。中途 wait / revision / blocked 状态未在 M2 编译恢复分支，必须在写 shadow 前诚实阻断。`validate-workflow-kernel-m2.ps1` 用 1 个空格中文路径正例和 15 个负例验证四面 parity、byte-stable rebuild、幂等 replay、Schema 形状、非正向状态、越界与篡改防护。它不调用语义 worker、网络或 provider，不写 current session，不切换 legacy R7，也不构成 runtime certification。
...
`WorkflowKernelSessionEntry.ps1` / `invoke-workflow-session-entry.ps1` 是 session 唯一代际入口。M5 新建 session 使用 v0.2 immutable binding 且禁止 `compatibility_catalog_sha256`；重复 start byte-stable，resume 只服从已提交 binding。没有 binding 但存在 version-pinned R7 plan 的旧 session 只通过 `WorkflowCompatibilityLoader.ps1` 只读解析为 `legacy_r7`，不补写 binding、不迁移旧 plan。回滚只改变未来新 session。`validate-workflow-kernel-m4.ps1` 保持 19 个代际入口回归；`validate-workflow-kernel-m5.ps1` 以 16 个正反 fixture 验证 current 热路径隔离、旧 v0.1 binding 兼容、loader-only、未知蓝图 / 路由 / 资产 fail-closed 和只读稳定性。两者均不构成 runtime certification。
