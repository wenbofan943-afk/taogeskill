...
try{
  ...
  $result=if($errors.Count){'blocked'}else{'baseline_prompt_evidence_ready_for_visual_need_analysis'}
  $report=[ordered]@{schema_id='taoge://reports/p0/h6-preflight/v0.3';schema_version='0.3';generated_at=[DateTimeOffset]::UtcNow.ToString('o');h5_session_id=(Split-Path -Leaf $h5);prompt_source_session_id=(Split-Path -Leaf $source);preflight_result=$result;provider_route='codex_builtin_image2';visual_count_policy='content_derived_unbounded';generation_policy='generate_all_accepted';accepted_task_dispatch_policy='auto_continue_all_accepted_without_human_confirmation';human_confirmation_required=$false;on_visual_need_pass='auto_continue_to_image_prompt_compiler_then_image_asset_producer';provider_call_limit=$null;cost_gate='not_applicable';baseline_prompt_evidence_count=$prompts.Count;accepted_visual_task_count=$null;external_calls_executed=0;prompts=[object[]]$prompts.ToArray();errors=[object[]]$errors.ToArray()}
