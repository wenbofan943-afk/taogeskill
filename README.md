...
给中文内容创作者使用的 AI 内容工作流 Skill。它把选题调研、内容 Brief、口播文案、画中画提示词 / 图片、文案质检、多平台发布物料和最终 HTML 交付页串成一条可接续的工作流。

适合：懂一点 AI / Codex / GitHub，想把内容生产流程沉淀下来的人。不需要写代码；下载后把项目文件夹交给 Codex 或其他能读本地文件的 AI，再按快速开始对话即可。

公开搜索关键词：`taogeskill`

最新版下载：[v0.1.0-alpha.6 GitHub Release](https://github.com/wenbofan943-afk/taogeskill/releases/tag/v0.1.0-alpha.6)

> Alpha 预发行提醒：当前公开包是 `0.1.0-alpha.6` GitHub 预发行版本，不是生产级自动化 runner；不能自动发布内容、登录平台、互动评论 / 私信，也不能证明真实传播效果。它已完成单篇真实内容生产、H7 最终交付、多宿主 Windows 验证和 R5 账号身份 / 热点雷达合同闭环，但外部 tester 验收仍待后续完成。
...
1. 本账号的长期“母题”是什么，也就是能持续产生内容的战术规划单元。
2. 当前绑定产品怎么对外表达，避免被误解为灰产工具、截流工具或批量获客工具。
3. 第一批自媒体栏目和选题。
4. 第一批短视频 / 图文脚本。
5. GitHub、官网、下载页和客户端首屏的一致口径。
6. 免费学习版申请转化路径。
7. 热点文案 skill 如何保持轻量脚本边界，并为未来短视频创作 SaaS 预留可迁移字段。
8. 账号档案和账号母题如何约束热点选取，避免不同账号混用同一套热点逻辑。
...
- [NOTICE.md](./NOTICE.md)：项目边界、外部方法论鸣谢和外部资料边界。
- [文档治理与目录规范](./docs/reference/文档治理与目录规范.md)：规定账号目录、session 目录、中间产物、最终交付物和 manifest 规则。
- [人类引导与任务后导航规范](./docs/reference/人类引导与任务后导航规范.md)：规定什么时候必须停、什么时候自动继续，以及如何给出有理由的下一步。
...
- [账号档案完整性检查表](./docs/reference/账号档案完整性检查表.md)
- [账号母题与传播工作流](./docs/reference/账号母题与传播工作流.md)
- [产品与活动对象档案](./docs/reference/产品与活动对象档案.md)
- [AI热点发现与关联评估方法论](./docs/reference/AI热点发现与关联评估方法论.md)
- [热点搜索来源池](./docs/reference/热点搜索来源池.md)
- [热点候选池](./docs/reference/热点候选池.md)
- [调研运行记录](./docs/reference/调研运行记录.md)
...
- [文案策略矩阵](./docs/reference/文案策略矩阵.md)
- [热点文案 Skill 方法论与 SaaS 承接设计](./docs/reference/热点文案Skill方法论与SaaS承接设计.md)
...
├── docs/reference/热点研究CLI与API工具策略.md
...
8. 新增账号时，必须先按 [账号档案完整性检查表](./docs/reference/账号档案完整性检查表.md) 补齐 P0 字段；P0 不齐不进入热点创作。
...
4. 不承诺任何未验证的商业化结论。
5. 不做自动发布工具。
6. 不接抖音、小红书、视频号、B站等平台发布 API。
7. 不自动登录平台后台。
8. 不自动评论、私信、互动或抓取平台后台数据。
...
每轮热点研究还必须先生成来源计划，来源从 [热点搜索来源池](./docs/reference/热点搜索来源池.md) 中选：
...
涛哥创作工作流：当前轻量内容生产和传播验证落点，服务涛哥账号矩阵和后续产品。
```
...
本项目已作为 GitHub Alpha 预发行包公开；它仍是轻量、单篇内容 workflow，不是自动发布平台或内容生产 SaaS。
