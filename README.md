...
最新版下载：[v0.1.0-alpha.11 GitHub Release](https://github.com/wenbofan943-afk/taogeskill/releases/tag/v0.1.0-alpha.11)

> Alpha 预发行提醒：当前公开包是 `0.1.0-alpha.11` GitHub 预发行版本，不是生产级自动化 runner。它不能自动发布内容、登录平台、互动评论 / 私信，也不能证明真实传播效果。当前 Windows 正式兼容基线是系统自带的 Windows PowerShell 5.1；PowerShell 7 不属于当前公开承诺。本版保留公开脱敏的直供与热点交付链离线复验，并将第三方调研资料移出 GitHub 源码归档，以公开源码边界门禁防止回归；真实运行仍有 Codex 语义与视觉判断，整项目保持 L2.8。外部 tester 验收仍待后续完成。
